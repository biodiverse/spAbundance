lfMsDS <- function(abund.formula, det.formula, data, inits, priors, tuning,
                   n.factors, n.batch, batch.length, accept.rate = 0.43, 
  ...
  if (length(dim(data$y)) != 3) {
    stop("error: count data y must be a three-dimensional array with dimensions corresponding to species, sites, and replicates.")
  }
  y <- data$y
  y.mat <- y
  ...
  if (!k.fold.only) {
    for (i in 1:n.chains) {
      ...
      storage.mode(chain.info) <- "integer"
      # Run the model in C
      out.tmp[[i]] <- .Call("lfMsDS", y, X, X.p, X.re, X.p.re, X.random, X.p.random, 
  ...
  if (!missing(k.fold)) {
	...
	p.re.level.names.fit <- p.re.level.names
      ...
      storage.mode(chain.info) <- "integer"
      # Run the model in C
      out.fit <- .Call("lfMsDS", y.fit, X.fit, X.p.fit, X.re.fit, X.p.re.fit,
		       ...
		       beta.comm.inits, alpha.comm.inits, 
		       lambda.inits, w.inits, tau.sq.beta.inits, tau.sq.alpha.inits,
      ...
      out.pred <- predict.lfMsDS(out.fit, X.0, coords.0, k.fold.offset = offset.0)
      ...
      out.p.pred <- predict.lfMsDS(out.fit, X.p.0, type = 'detection')
