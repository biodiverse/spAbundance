svcAbund <- function(formula, data, inits, priors, tuning, 
		    ...
		    n.thin = 1, n.chains = 1, stage.1.like, ...){
  ...
  if (!(family) %in% c('Gaussian', 'Gaussian-hurdle')) {
    stop("svcAbund currently only supports family = 'Gaussian' or 'Gaussian-hurdle'")
  }
  ...
  if (two.stage) {
    if (missing(stage.1.like)) {
      message("NOTE: stage.1.like not specified for Gaussian-hurdle model.\nResulting likelihood values and WAIC will not be correct.\nI would suggest rerunning the model with the stage 1 likelihood values supplied.")
    } else {
