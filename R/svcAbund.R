svcAbund <- function(formula, data, inits, priors, tuning, svc.cols = 1,
  ...
  if (family %in% c('Gaussian', 'zi-Gaussian')) {
    ...
    if (is.character(svc.cols)) {
      ...
      if (!all(svc.cols %in% x.names)) {
        ...
        stop(paste("error: variable name ",
                   paste(missing.cols, collapse=" and "), " not inurrence covariates", sep=""))
      }
      ...
      if (!all(svc.cols %in% 1:p.abund)) {
        ...
        stop(paste("error: column index ",
                   paste(missing.cols, collapse=" "), " not in design matrix columns", sep=""))
      }
    }
    ...
    if (!NNGP) {
      stop("svcAbund is currently only implemented with NNGPs. Please set NNGP = TRUE")
    } else {
      ...
      consts <- c(J, n.obs, p.abund, p.abund.re, n.abund.re, save.fitted, p.svc)
      ...
      for (i in 1:n.chains) {
        ...
        if ((i > 1) & (!fix.inits)) {
          ...
          sigma.sq.inits <- runif(p.svc, 0.05, 3)
          phi.inits <- runif(p.svc, phi.a, phi.b)
          if (cov.model == 'matern') {
            nu.inits <- runif(p.svc, nu.a, nu.b)
          }
        ...
        storage.mode(chain.info) <- "integer"
        # Run the model in C
        out.tmp[[i]] <- .Call("svcAbundNNGP", y, X, X.w, coords, X.re, X.random,
