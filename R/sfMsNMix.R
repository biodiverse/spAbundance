sfMsNMix <- function(abund.formula, det.formula, data, inits, priors,  
  ...
  if (!NNGP) {
    stop("error: sfMsNMix is currently only implemented for NNGPs, not full Gaussian Processes. Please set NNGP = TRUE.") 
  }
  if (missing(data)) {
  ...
  if (nrow(X.p) == length(y) / n.sp) {
    X.p <- X.p[!is.na(c(y.mat[1, , ])), , drop = FALSE]
  }
  if (nrow(X.p.re) == length(y) / n.sp & p.det.re > 0) {
    X.p.re <- X.p.re[!is.na(c(y.mat[1, , ])), , drop = FALSE]
  }
  y <- y[!is.na(y)]
  ...
  if (!NNGP)  {
    ...
    for (i in 1:n.chains) {
      ...
      storage.mode(chain.info) <- "integer"
      out.tmp[[i]] <- .Call("sfMsNMixNNGP", y, X, X.p, coords, X.re, X.p.re, y.max, consts, 
			    ...
			    phi.inits, lambda.inits, nu.inits, w.inits,
        		    ...
        		    Sigma.beta.comm, Sigma.alpha.comm, kappa.a, 
          		    kappa.b, tau.sq.beta.a, tau.sq.beta.b, tau.sq.alpha.a, 
        	            tau.sq.alpha.b, phi.a, phi.b, nu.a, nu.b, 
    ...
    out$N.samples <- out$N.samples[, order(ord), ]
    ...
    #     out.fit <- .Call("sfMsNMixNNGP", y.fit, X.fit, X.p.fit, X.re.fit, X.p.re.fit, consts.fit, 
    ...
    #     out.pred <- predict.sfMsNMix(out.fit, X.0)
