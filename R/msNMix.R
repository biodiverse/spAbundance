msNMix <- function(abund.formula, det.formula, data, inits, priors,  
		   tuning, n.batch, batch.length, accept.rate = 0.43, family = 'NB', 
  ...
  for (i in 1:n.chains) {
    ...
    storage.mode(chain.info) <- "integer"
    out.tmp[[i]] <- .Call("msNMix", y, X, X.p, X.re, X.p.re, y.max, consts, 
  ...
  #     out.fit <- .Call("msNMix", y.fit, X.fit, X.p.fit, X.re.fit, X.p.re.fit, consts.fit, 
