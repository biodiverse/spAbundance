svcMsAbund <- function(formula, data, inits, priors, 
                  tuning, svc.cols = 1, joint.cols = 1, 
		  cov.model = 'exponential', NNGP = TRUE, 
                  n.neighbors = 15, search.type = "cb", n.factors, 
		  family = 'Gaussian',
                  n.batch, batch.length, accept.rate = 0.43, adapt = TRUE,
  ...
  if (length(n.factors) != 1 & length(n.factors) != length(svc.cols)) {
    stop(paste("n.factors must be of length 1 or ", length(svc.cols), sep = ''))
  }
  # TODO: left off updating here.
  ...
  # Check dimensions of first-stage sample
  if (two.stage) {
    if (length(dim(z)) != 2) {
      stop(paste("error: z must be a two-dimensional array with dimensions of ", 
		 N, " species, and ", J, " sites.", sep = ''))
    }
    if (dim(z)[1] != N | dim(z)[2] != J) {
      stop(paste("error: z must be a two-dimensional array with dimensions of ", 
		 N, " species, and ", J, " sites.", sep = ''))
    }
    # Reorder z
  ...
  if (is.character(svc.cols)) {
    ...
    if (!all(svc.cols %in% x.names)) {
        ...
        stop(paste("error: variable name ", paste(missing.cols, collapse=" and "), " not in covariates", sep=""))
    }
  ...
  if ("w" %in% names(inits)) {
    ...
    for (i in 1:p.svc) {
      if (nrow(w.inits[[i]]) != q | ncol(w.inits[[i]]) != J) {
        stop(paste("error: initial values for w[[", i, 
		   "]] must be a matrix with dimensions ", q, " x ", J, sep = ""))
      }
      # Reorder elements of w.inits supplied by user
      if (NNGP) {
  ...
  # Names for spatial params in model object
  if (cov.model != 'matern') {
  ...
  if (!NNGP) {
    ...
    # TODO: testing
    # beta.comm.inits <- beta.mean
    # tau.sq.beta.inits <- tau.sq.beta
    # phi.inits <- phi
    # lambda.inits <- unlist(dat$lambda)
    # w.inits <- unlist(lapply(dat$w, function(a) a[, ord]))
    # beta.inits <- beta
    ...
    for (i in 1:n.chains) {
      ...
      # Run the model in C
      if (two.stage) {
        out.tmp[[i]] <- .Call("svcMsAbundTwoStageNNGP", y, X, X.w, coords, X.re, consts, n.re.long, 
        		         ...
        		         tuning.c, cov.model.indx, n.batch, 
        	                 ...
        	                 samples.info, chain.info, z, adapt)
      } else {
        out.tmp[[i]] <- .Call("svcMsAbundNNGP", y, X, X.w, coords, X.re, consts, n.re.long, 
        	                n.neighbors, nn.indx, nn.indx.lu, u.indx, u.indx.lu, ui.indx,
        	                beta.inits, beta.comm.inits, tau.sq.beta.inits, tau.sq.inits,
        	                phi.inits, lambda.inits, nu.inits, w.inits, sigma.sq.mu.inits, 
        		        beta.star.inits, beta.star.indx, beta.level.indx, mu.beta.comm, 
