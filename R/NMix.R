NMix <- function(abund.formula, det.formula, data, inits, priors, tuning,
  ...
  for (i in 1:n.chains) {
    ...
    storage.mode(chain.info) <- "integer"
    # Run the model in C
    out.tmp[[i]] <- .Call("NMix", y, X, X.p, X.re, X.p.re, y.max, consts, K, n.abund.re.long, 
  ...
  if (!missing(k.fold)) {
    ...
    model.deviance <- foreach (i = 1:k.fold, .combine = sum) %dopar% {
      ...
      storage.mode(chain.info) <- "integer"
      # Run the model in C
      out.fit <- .Call("NMix", y.fit, X.fit, X.p.fit, X.re.fit, X.p.re.fit, y.max.fit, consts.fit, 
      		 K.fit, n.abund.re.long.fit, n.det.re.long.fit, beta.inits, alpha.inits, kappa.inits,
		 ...
		 Sigma.beta, mu.alpha, Sigma.alpha, sigma.sq.mu.a, sigma.sq.mu.b, 
      		 sigma.sq.p.a, sigma.sq.p.b, kappa.a, kappa.b, 
		 tuning.c, n.batch, batch.length, accept.rate, n.omp.threads.fit, verbose.fit, 
      ...
      for (j in 1:nrow(X.p.0)) {
        like.samples[j] <- mean(dbinom(y.0[j], out.pred$N.0.samples[, N.0.long.indx[j]], 
      			   p.0.samples[, j]))
      }
