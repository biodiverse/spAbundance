ppcAbund <- function(object, fit.stat, group, type = 'marginal', ...) {
  ...
  if (!(class(object) %in% c('NMix', 'spNMix', 'abund', 'spAbund', 
           ...
           'svcMsAbund', 'svcAbund'))) {
    stop("error: object must be one of the following classes: NMix, spNMix, abund, spAbund, msAbund, lfMsAbund, sfMsAbund, msNMix, lfMsNMix, sfMsNMix, DS, spDS, msDS, lfMsDS, sfMsDS, svcMsAbund, svcAbund")
  }
  # Fit statistic ---------------------
  if (missing(fit.stat)) {
