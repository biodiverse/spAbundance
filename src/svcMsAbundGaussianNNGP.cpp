...
extern "C" {
  SEXP svcMsAbundGaussianNNGP(SEXP y_r, SEXP X_r, SEXP Xw_r, SEXP coords_r, SEXP XRE_r,
    ...
    PROTECT(lambdaSamples_r = Rf_allocMatrix(REALSXP, NqpTilde, nPost)); nProtect++;
    zeros(REAL(lambdaSamples_r), NqpTilde * nPost);
    SEXP wSamples_r;
    PROTECT(wSamples_r = Rf_allocMatrix(REALSXP, JqpTilde, nPost)); nProtect++;
    ...
    PROTECT(thetaSamples_r = Rf_allocMatrix(REALSXP, nThetaqpTildeSave, nPost)); nProtect++;
    ...
    PROTECT(tuningSamples_r = Rf_allocMatrix(REALSXP, nThetaqpTilde, nBatch)); nProtect++;
...
#pragma omp parallel for private (e, ii, b) reduction(+:aa, logDet)
    ...
    SET_VECTOR_ELT(resultName_r, 8, Rf_mkChar("tau.sq.samples"));
