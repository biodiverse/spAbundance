...
void updateBFsvcAbund(double *B, double *F, double *c, double *C, double *coords, int *nnIndx, int *nnIndxLU, int n, int m, double sigmaSq, double phi, double nu, int covModel, double *bk, double nuUnifb){
...
extern "C" {
  SEXP svcAbundNNGP(SEXP y_r, SEXP X_r, SEXP Xw_r, SEXP coords_r, SEXP XRE_r,
    ...
    ********************************************************************/
    ...
    for (i = 0; i < pTilde; i++) {
      updateBFsvcAbund(&B[i * nIndx], &F[i * J], &c[i * m * nThreads],
    ...
    double *wSites = (double *) R_alloc(nObs, sizeof(double));
    double *wSitesCand = (double *) R_alloc(nObs, sizeof(double));
    // For each location, multiply w x Xw
    for (i = 0; i < nObs; i++) {
      wSites[i] = 0.0;
      for (ll = 0; ll < pTilde; ll++) {
        wSites[i] += w[siteIndx[i] * pTilde + ll] * Xw[ll * nObs + i];
      }
      wSitesCand[i] = wSites[i];
    }
    ...
    ********************************************************************/
    ...
    int nAMCMC = 0;
    if (pAbundRE > 0) {
      nAMCMC = nThetapTilde + pAbund + JpTilde + nAbundRE;
    } else {
      nAMCMC = nThetapTilde + pAbund + JpTilde;
    }
    if (family == 1) {
      nAMCMC++;
    }
        ...
        for (ll = 0; ll < pTilde; ll++) {
          ...
          for (j = 0; j < J; j++) {
            ...
            for (i = 0; i < nObs; i++) {
              if (siteIndx[i] == j) {
                ...
                for (rr = 0; rr < pTilde; rr++) {
                  wSitesCand[i] += wCand[j * pTilde + rr] * Xw[rr * nObs + i];
                }
                tmp_nObs[i] = exp(F77_NAME(ddot)(&pAbund, &X[i], &nObs, beta, &inc) +
                                  betaStarSites[i] + wSitesCand[i]);
                if (family == 1) {
                  logPostWCand[j * pTilde + ll] += dnbinom_mu(y[i], kappa, tmp_nObs[i] * offset[i], 1);
                } else {
            ...
            for (i = 0; i < nObs; i++) {
              if (siteIndx[i] == j) {
                tmp_nObs[i] = exp(F77_NAME(ddot)(&pAbund, &X[i], &nObs, beta, &inc) +
                                  betaStarSites[i] + wSites[i]);
                if (family == 1) {
                  logPostWCurr[j * pTilde + ll] += dnbinom_mu(y[i], kappa, tmp_nObs[i] * offset[i], 1);
                } else {
...
#pragma omp parallel for private (e, i, b) reduction(+:a)
          ...
          updateBFsvcAbund(&B[ll * nIndx], &F[ll * J], &c[ll * m * nThreads],
...
#pragma omp parallel for private (e, i, b) reduction(+:a, logDet)
          ...
          updateBFsvcAbund(BCand, FCand, &c[ll * m * nThreads], &C[ll * mm * nThreads], 
