...
extern "C" {
  SEXP svcAbundNNGP(SEXP y_r, SEXP X_r, SEXP Xw_r, SEXP coords_r, SEXP XRE_r, SEXP XRandom_r,
    ...
    SEXP likeZeroSamples_r;
    PROTECT(likeZeroSamples_r = allocMatrix(REALSXP, JZero, nPost)); nProtect++;
    ...
    double *likeZero = (double *) R_alloc(JZero, sizeof(double)); zeros(likeZero, JZero);
...
#pragma omp parallel for private (e, ii, b) reduction(+:aa, logDet)
	...
	if (q >= nBurn) {
	  ...
	  if (thinIndx == nThin) {
	    ...
	    F77_NAME(dcopy)(&JZero, yRepZero, &inc, &REAL(yRepZeroSamples_r)[sPost*JZero], &inc); 
            ...
            F77_NAME(dcopy)(&JZero, likeZero, &inc, 
        		    &REAL(likeZeroSamples_r)[sPost*JZero], &inc);
	    sPost++; 
	    thinIndx = 0; 
	  }
	}
    ...
    SET_VECTOR_ELT(result_r, 9, likeZeroSamples_r); 
    SET_VECTOR_ELT(result_r, 10, yRepZeroSamples_r); 
    if (pRE > 0) {
    ...
    SET_VECTOR_ELT(resultName_r, 9, mkChar("like.zero.samples")); 
    SET_VECTOR_ELT(resultName_r, 10, mkChar("y.rep.zero.samples")); 
    if (pRE > 0) {
