...
extern "C" {
  ...
  SEXP spAbundNNGPPredict(SEXP coords_r, SEXP J_r, SEXP nObs_r, 
    ...
    if(verbose){
      ...
      Rprintf("----------------------------------------\n");
      Rprintf("NNGP spatial abundance model fit with %i observations.\n\n", J);
    ...
    PROTECT(y0_r = allocMatrix(REALSXP, nObs0, nSamples)); nProtect++; 
    PROTECT(mu0_r = allocMatrix(REALSXP, nObs0, nSamples)); nProtect++; 
    PROTECT(w0_r = allocMatrix(REALSXP, J0, nSamples)); nProtect++;
    double *y0 = REAL(y0_r);
