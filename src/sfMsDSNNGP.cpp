...
extern "C" {
  SEXP sfMsDSNNGP(SEXP y_r, SEXP X_r, SEXP Xp_r, SEXP coords_r, SEXP XRE_r, SEXP XpRE_r,
    ...
    ********************************************************************/
    ...
    SEXP yRepSamples_r;
    PROTECT(yRepSamples_r = Rf_allocMatrix(INTSXP, nObsFullnSp, nPost)); nProtect++;
    zerosInt(INTEGER(yRepSamples_r), nObsFullnSp * nPost);
    SEXP piFullSamples_r;
    PROTECT(piFullSamples_r = Rf_allocMatrix(REALSXP, nObsFullnSp, nPost)); nProtect++;
...
#pragma omp parallel for private (e, ii, b) reduction(+:aa, logDet)
    ...
    SET_VECTOR_ELT(resultName_r, 8, Rf_mkChar("tuning.samples"));
    SET_VECTOR_ELT(resultName_r, 9, Rf_mkChar("y.rep.samples"));
    SET_VECTOR_ELT(resultName_r, 10, Rf_mkChar("pi.samples"));
    SET_VECTOR_ELT(resultName_r, 11, Rf_mkChar("lambda.samples"));
    SET_VECTOR_ELT(resultName_r, 12, Rf_mkChar("w.samples"));
    SET_VECTOR_ELT(resultName_r, 13, Rf_mkChar("theta.samples"));
    if (pDetRE > 0) {
      SET_VECTOR_ELT(resultName_r, 14, Rf_mkChar("sigma.sq.p.samples"));
      SET_VECTOR_ELT(resultName_r, 15, Rf_mkChar("alpha.star.samples"));
    }
    if (pAbundRE > 0) {
