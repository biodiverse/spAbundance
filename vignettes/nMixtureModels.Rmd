...
\begin{equation}
\begin{split}
  \hat{N}_{j} &\sim \text{Poisson}(\mu_j), \\
...
Notice the Rhat values are only reported for the elements of the factor loadings matrix that are estimated and not fixed at any specific value, while the ESS values are reported for all elements of the factor loadings matrix, and take value 0 for those parameters that are fixed for identifiability purposes. We see fairly small ESS values and some Rhat values above 1.1, so we would run this model for longer in a complete analysis. We can inspect the latent factor loadings as well as the latent factors (stored in `out.lf.ms$w.samples`) to provide information on any groupings that arise from the species in the modeled community. See Appendix S1 in @doser2023joint for a discussion on using factor models as a model-based ordination technique. 
