...
For N-mixture models, the `ppcAbund()` function contains an additional argument `type`, which is used to indicate the specific form of fitted value to generate from the given model. This argument can take two values: `marginal` and `conditional`. When `type = 'conditional'`, the fitted values are generated conditional on the observed data points, while when `type = 'marginal'`, the fitted values are not generated conditional on the observed data points. More specifically, for the conditional fitted values, the fitted value at site $j$ and replicate $k$, denoted as $\hat{y}_{j, k}$ is calculated according to 
