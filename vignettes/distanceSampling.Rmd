...
where $\bm{\beta}$ is a vector of effects for covariates in $\textbf{x}_j$ (including an intercept)

We assume data come from observers that count the number of individuals of the species at any given location (i.e., a line transect or point count survey) and record the distance to the line/center of the point count survey of each bird within a set of $k = 1, \dots, K$ distance bands. Note that if continuous distances are reported, they can be post-hoc binned into a set of $K$ distance bands and fit with the package. Define $\bm{y}_j$ as a vector of $K$ values indicating the number of individuals observed within each of the $k$ distance bands at site $j$. Similarly, let $\bm{y}^\ast_j$ be a vector of $K + 1$ values, where the first $K$ values correspond to $\bm{y}_j$, and the last value is the number of unobserved individuals at that location (i.e., $N_j - \sum_{k = 1}^Ky_{k, j}$.) Note that the last value in $\bm{y}^\ast_j$ is not observed (i.e., since $N_j$ is not known). We model $\bm{y}^\ast_j$ according to 

\begin{equation}
  \bm{y}^\ast_j \sim \text{Multinomial}(N_j, \bm{\pi}_j^\ast),
\end{equation}

where $\bm{\pi}_j^\ast$ is a vector of cell-specific detection probabilities, where $\pi_{j, K + 1} = 1 - \sum_{k = 1}^K\pi_{j, k}$, and $\bm{\pi}_j$ are the first $K$ values in $\bm{\pi}_j^\ast$. More specifically, $\pi_{j, k}$ is the probability of detecting an individual in the $k$th distance band at site $j$. We define $\pi_{j, k}$ as 

\begin{equation}
  \pi_{j, k} = \bar{p}_{j, k}\psi_{k},
\end{equation}

where $\bar{p}_{j, k}$ is the probability of detecting an individual in distance band $k$, given the individual occurs in distance band $k$, and $\psi_{k}$ is the probability an individual occurs in distance band $k$. Following the standard distance sampling assumption that animals are uniformly distributed in space, for line transects, we have
...
\begin{equation}
  \bar{p}_{j, k} = \frac{1}{B}\int_{b_k}^{b_{k+1}}g(x)dx.
\end{equation}
...
\begin{equation}
  \bar{p}_{j, k} = \frac{1}{B^2}\int_{b_k}^{b_{k+1}}g(x)2xdx.
\end{equation}

For both line transects and point count surveys, $g(x)$ is some function of distance $x$ from the transect line/center of point count survey. `spAbundance` currently supports two detection functions: half-normal and negative exponential. Both of these functions are governed by a scale parameter $\sigma_j$, which can vary across sites to allow detection probability to vary across spatial locations. We can then model $\sigma_j$ as 

\begin{equation}
  \text{log}(\sigma_j) = \textbf{v}_j^\top\bm{\alpha},
\end{equation}

where $\bm{\alpha}$ is a vector of regression coefficients for covariates $\textbf{V}$ (including an intercept). 

To complete the Bayesian specification of the models, we assign normal priors to the abundance ($\bm{\beta}$) and detection ($\bm{\alpha}$) regression coefficients, and a uniform prior for the negative binomial overdispersion parameters (if applicable).
...
Notice the `dist.breaks` element in `dat.EATO` consists of six values: 

```{r}
dat.EATO$dist.breaks
```

These values correspond to the distance cut off values (in km) for the 4 distance bins in our distance sampling data. An important point to mention is that when fitting distance sampling models in `spAbundance`, the models can be somewhat sensitive to initial values of the detection parameters. This is particularly true when the distance breaks take large values (e.g., 100). Thus, we recommend converting the units of distance breaks to a value that is as small as possible, but still makes sense. For example, we originally calculated the distance breaks values for this data set in meters, but we then divided them by 1000 to give the distance break values in km instead. 

The `offset` in this case takes value `r round(dat.EATO$offset, 2)`), which will put the estimated regression coefficients on the scale of individuals per ha rather than individuals per point count. To see where the value comes from, recall we are using 250m (.25km) radius point count surveys and that the area of a circle is $\pir^2$, with $r$ being the circle's radius. To get our estimates on a per hectare scale, we set our offset to the area in km$^2$ then multiply it by 100.
...
Note that when the model gets "stuck" on an initial value (which can happen if supplying an initial starting value for the detection parameters that is very far from the estimated value), the acceptance rate of the parameter will be stuck at 0, and will not change throughout the algorithm. This is a clear indicator that we need to give closer attention to our initial values.
...
`DS()` returns a list of class `DS` with a suite of different objects, many of them being `coda::mcmc` objects of poseterior samples. The "Preparing to run the model" section will print information on default priors or initial values that are used when they are not specified in the function call. Here we specified everything explicitly so no information was reported.
...
There are some interesting differences between the predictions of the spatial and non-spatial model, which we'll leave to you to explore if you desire. Looking at the predictions of the spatial model, we immediately see two things: (1) the uncertainty is substantially larger than that of the non-spatial model; and (2) the uncertainty decreases as one gets farther away from the observed locations. The latter point is a common phenomena we will observe when fitting any spatial model, and the degree to which the uncertainty increases as one gets farther away from sampled locations will depend on the estimated effective spatial range. 
...
ggplot(data = comm.plot.df) +
  geom_ribbon(aes(x = x.val, ymin = low, ymax = high), fill = 'grey',
	      alpha = 0.5) +
  geom_line(data = p.plot.df, aes(x = x.val, y = val, col = sp), size = 1, lty = 1) +
  theme_bw(base_size = 14) +
  geom_line(aes(x = x.val, y = mean), col = 'black', size = 1.3) +
  theme(text = element_text(family="LM Roman 10")) + 
...
Prediction proceeds exactly as we have seen previously with the single-species non-spatial HDS model.

``{r, echo = FALSE}
# Clean things up so this whole thing can run.
rm(out.pred, out.sp.pred, out, out.sp)
gc()
```
...
Notice that the predictions of average abundance (`mu.0.samples`) and latent abundance (`N.0.samples`) are now three-dimensional arrays, with the dimensions corresponding to MCMC sample, species, and site. We can generate plots of species-specific abundance as we have before, or we could generate some sort of community wide summary using either the expected abundance or latent abundance measures. For example, we could imagine summarizing the latent abundance values to get estimates of species richness, or perhaps some abundance-weighted metric of diversity. Alternatively, we could plot the total expected abundance summed across all species as a simple measure of "how many birds" we could expect at any given location. We generate this plot below. 
...
Not surprisingly, this plot is pretty similar to the plot for EATO, which is the most common bird in the data set, and thus it's spatial variation in abundance contributes the most to the spatial variation in total abundance across the group of species. 

```{r, echo = FALSE}
rm(mu.sum.samples, out.ms.pred)
gc()
```
...
For guidance on choosing the number of latent factors, see [the section on fitting latent factor multi-species occupancy models](https://www.jeffdoser.com/files/spoccupancy-web/articles/factormodels#latent-factor-multi-species-occupancy-models) on the `spOccupancy` website. Here we will fit a model with 3 latent factors. 
...
We can inspect the latent factor loadings as well as the latent factors (stored in `out.lf.ms$w.samples`) to provide information on any groupings that arise from the species in the modeled community. See Appendix S1 in @doser22joint for a discussion on using factor models as a model-based ordination technique.
...
We have already discussed all the components required for fitting models with `sfMsDS()` as it has the same arguments as `lfMSDS()` as well as the additional arguments for fitting spatial models that we saw with `spDS()`. Below we run all the code for fitting the same model as we did with `lfMsDS()`, but now accommodating spatial autocorrelation in the latent factor. Notice the main difference in preparing the model arguments for fitting a model with `sfMsDS()` compared to `lfMsDS()` is the addition of the spatial decay parameters `phi` in the initial values, priors, and tuning variances. Note that out of all the models discussed, `sfMsDS()` is the slowest model (which makes sense since it is also the most complicated).
