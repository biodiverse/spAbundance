# Test spNMix.R -----------------------------------------------------------

skip_on_cran()

# Intercept only ----------------------------------------------------------
set.seed(1000)
J.x <- 15
J.y <- 15
J <- J.x * J.y
n.rep <- sample(2:4, J, replace = TRUE)
beta <- c(0)
p.abund <- length(beta)
alpha <- c(0.5)
p.det <- length(alpha)
mu.RE <- list()
p.RE <- list()
phi <- runif(1, 3 / 1, 3 / .1)
sigma.sq <- 0.5
kappa <- 0.5
phi <- 3 / 0.5
nu <- 1
sp <- TRUE
cov.model <- 'matern'
family <- 'NB'
dat <- simNMix(J.x = J.x, J.y = J.y, n.rep = n.rep, beta = beta, alpha = alpha,
	       mu.RE = mu.RE, p.RE = p.RE, sp = TRUE, family = family, kappa = kappa, 
               sigma.sq = sigma.sq, phi = phi, nu = nu, cov.model = cov.model)

pred.indx <- sample(1:J, round(J * .25), replace = FALSE)
y <- dat$y[-pred.indx, , drop = FALSE]
y.0 <- dat$y[pred.indx, , drop = FALSE]
# Occupancy covariates
X <- dat$X[-pred.indx, , drop = FALSE]
# X.re <- dat$X.re[-pred.indx, , drop = FALSE]
# Prediction covariates
X.0 <- dat$X[pred.indx, , drop = FALSE]
# X.re.0 <- dat$X.re[pred.indx, , drop = FALSE]
# Detection covariates
X.p <- dat$X.p[-pred.indx, , , drop = FALSE]
# X.p.re <- dat$X.p.re[-pred.indx, , , drop = FALSE]
coords <- as.matrix(dat$coords[-pred.indx, ])
coords.0 <- as.matrix(dat$coords[pred.indx, ])

abund.covs <- X
colnames(abund.covs) <- c('int')
det.covs <- list(int = X.p[, , 1])
data.list <- list(y = y,
		  abund.covs = abund.covs,
 		  det.covs = det.covs, 
                  coords = coords)
# Priors
prior.list <- list(beta.normal = list(mean = 0, var = 10),
		   alpha.normal = list(mean = 0, var = 2.72),
                   sigma.sq.ig = c(2, 1),
                   phi.unif = c(3 / 1, 3 / .1), 
                   nu.unif = c(0.1, 5)) 

# Starting values
inits.list <- list(alpha = 0,
		   beta = 0,
		   kappa = 1.3,
		   sigma.sq.mu = 0.5,
		   N = apply(y, 1, max, na.rm = TRUE), 
                   phi = 3 / .5, sigma.sq = 0.5, nu = 2)
tuning <- list(beta = 0.1, alpha = 0.1, beta.star = 0.1, alpha.star = 0.1,
               kappa = 0.2, phi = 0.5, sigma.sq = 0.5, nu = 0.5, w = 1)
n.batch <- 10
batch.length <- 25
n.burn <- 50
n.thin <- 2
n.chains <- 2
abund.formula <- ~ 1
det.formula <- ~ 1

out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      inits = inits.list,
	      family = 'NB',
	      tuning = tuning,
	      priors = prior.list,
	      cov.model = 'matern', 
	      NNGP = TRUE, 
	      n.neighbors = 5,
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 1,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains,
              k.fold = 2,
              k.fold.threads = 1)

# Test to make sure it worked ---------
test_that("out is of class spNMix", {
  expect_s3_class(out, "spNMix")
})

# Check cross-validation --------------
test_that("cross-validation works", {
  expect_equal(length(out$rmspe), 1)
  expect_type(out$rmspe, "double")
  expect_gt(out$rmspe, 0)
})

# Check random effects ----------------
test_that("random effects are empty", {
  expect_equal(out$pRE, FALSE)
  expect_equal(out$muRE, FALSE)
})

# Check output data output is correct -
test_that("out$y == y", {
  expect_equal(out$y, y)
})

# Check Poisson -----------------------
test_that("Poisson works", {
  out <- spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'spherical',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = FALSE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1)
  expect_s3_class(out, "spNMix")
})

# Check default priors ----------------
test_that("default priors, multiple chains, inits, burn, thin work", {
  out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      family = 'NB',
	      NNGP = TRUE,
	      n.neighbors = 5,
	      cov.model = 'exponential',
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 50,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains)
  expect_s3_class(out, 'spNMix')
})

# Check summary -----------------------
test_that("summary works", {
  expect_output(summary(out))
})

# Check verbose -----------------------
test_that("verbose prints to the screen", {
  expect_output(spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'gaussian',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = TRUE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1))
})

# Check waicAbund -----------------------
test_that("waicAbund works for spNMix", {
  # as.vector gets rid of names
  waic.out <- as.vector(waicAbund(out, max(out$N.samples) + 10))
  expect_equal(length(waic.out), 3)
  expect_equal(waic.out[3], -2 * (waic.out[1] - waic.out[2]))
})

# Check fitted ------------------------
test_that("fitted works for spNMix", {
  fitted.out <- fitted(out)
  expect_equal(length(fitted.out), 2)
})

# Check predictions -------------------
test_that("predict works for spNMix", {
  pred.out <- predict(out, X.0, coords.0, verbose = FALSE)
  expect_type(pred.out, "list")
  expect_equal(dim(pred.out$mu.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
  expect_equal(dim(pred.out$N.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
})
test_that("detection prediction works", {
  J.str <- 100
  X.p.0 <- matrix(1, nrow = J.str, ncol = p.det)
  pred.out <- predict(out, X.p.0, type = 'detection')
  expect_type(pred.out, 'list')
  expect_equal(dim(pred.out$p.0.samples), c(out$n.post * out$n.chains, J.str))
})

# Check PPCs --------------------------
test_that("posterior predictive checks work for spNMix", {
  n.post.samples <- out$n.post * out$n.chains
  ppc.out <- ppcAbund(out, 'chi-square', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))
  
  ppc.out <- ppcAbund(out, 'chi-square', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))

  ppc.out <- ppcAbund(out, 'chi-square', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))
})

# Abundance covariate only ------------------------------------------------
set.seed(1000)
J.x <- 15
J.y <- 15
J <- J.x * J.y
n.rep <- sample(2:4, J, replace = TRUE)
beta <- c(0, 0.2, -0.3)
p.abund <- length(beta)
alpha <- c(0.5)
p.det <- length(alpha)
mu.RE <- list()
p.RE <- list()
phi <- runif(1, 3 / 1, 3 / .1)
sigma.sq <- 0.5
kappa <- 0.5
phi <- 3 / 0.5
nu <- 1
sp <- TRUE
cov.model <- 'matern'
family <- 'NB'
dat <- simNMix(J.x = J.x, J.y = J.y, n.rep = n.rep, beta = beta, alpha = alpha,
	       mu.RE = mu.RE, p.RE = p.RE, sp = TRUE, family = family, kappa = kappa, 
               sigma.sq = sigma.sq, phi = phi, nu = nu, cov.model = cov.model)

pred.indx <- sample(1:J, round(J * .25), replace = FALSE)
y <- dat$y[-pred.indx, , drop = FALSE]
y.0 <- dat$y[pred.indx, , drop = FALSE]
# Occupancy covariates
X <- dat$X[-pred.indx, , drop = FALSE]
# X.re <- dat$X.re[-pred.indx, , drop = FALSE]
# Prediction covariates
X.0 <- dat$X[pred.indx, , drop = FALSE]
# X.re.0 <- dat$X.re[pred.indx, , drop = FALSE]
# Detection covariates
X.p <- dat$X.p[-pred.indx, , , drop = FALSE]
# X.p.re <- dat$X.p.re[-pred.indx, , , drop = FALSE]
coords <- as.matrix(dat$coords[-pred.indx, ])
coords.0 <- as.matrix(dat$coords[pred.indx, ])

abund.covs <- X
colnames(abund.covs) <- c('int', 'abund.cov.1', 'abund.cov.2')
det.covs <- list(int = X.p[, , 1])
data.list <- list(y = y,
		  abund.covs = abund.covs,
 		  det.covs = det.covs, 
                  coords = coords)
# Priors
prior.list <- list(beta.normal = list(mean = 0, var = 10),
		   alpha.normal = list(mean = 0, var = 2.72),
                   sigma.sq.ig = c(2, 1),
                   phi.unif = c(3 / 1, 3 / .1), 
                   nu.unif = c(0.1, 5)) 

# Starting values
inits.list <- list(alpha = 0,
		   beta = 0,
		   kappa = 1.3,
		   sigma.sq.mu = 0.5,
		   N = apply(y, 1, max, na.rm = TRUE), 
                   phi = 3 / .5, sigma.sq = 0.5, nu = 2)
tuning <- list(beta = 0.1, alpha = 0.1, beta.star = 0.1, alpha.star = 0.1,
               kappa = 0.2, phi = 0.5, sigma.sq = 0.5, nu = 0.5, w = 1)
n.batch <- 10
batch.length <- 25
n.burn <- 50
n.thin <- 2
n.chains <- 2
abund.formula <- ~ abund.cov.1 + abund.cov.2
det.formula <- ~ 1

out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      inits = inits.list,
	      family = 'NB',
	      tuning = tuning,
	      priors = prior.list,
	      cov.model = 'matern', 
	      NNGP = TRUE, 
	      n.neighbors = 5,
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 50,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains,
              k.fold = 2,
              k.fold.threads = 1)

# Test to make sure it worked ---------
test_that("out is of class spNMix", {
  expect_s3_class(out, "spNMix")
})

# Check cross-validation --------------
test_that("cross-validation works", {
  expect_equal(length(out$rmspe), 1)
  expect_type(out$rmspe, "double")
  expect_gt(out$rmspe, 0)
})

# Check random effects ----------------
test_that("random effects are empty", {
  expect_equal(out$pRE, FALSE)
  expect_equal(out$muRE, FALSE)
})

# Check output data output is correct -
test_that("out$y == y", {
  expect_equal(out$y, y)
})

# Check Poisson -----------------------
test_that("Poisson works", {
  out <- spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'spherical',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = FALSE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1)
  expect_s3_class(out, "spNMix")
})

# Check default priors ----------------
test_that("default priors, multiple chains, inits, burn, thin work", {
  out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      family = 'NB',
	      NNGP = TRUE,
	      n.neighbors = 5,
	      cov.model = 'exponential',
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 50,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains)
  expect_s3_class(out, 'spNMix')
})

# Check summary -----------------------
test_that("summary works", {
  expect_output(summary(out))
})

# Check verbose -----------------------
test_that("verbose prints to the screen", {
  expect_output(spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'gaussian',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = TRUE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1))
})

# Check waicAbund -----------------------
test_that("waicAbund works for spNMix", {
  # as.vector gets rid of names
  waic.out <- as.vector(waicAbund(out, max(out$N.samples) + 10))
  expect_equal(length(waic.out), 3)
  expect_equal(waic.out[3], -2 * (waic.out[1] - waic.out[2]))
})

# Check fitted ------------------------
test_that("fitted works for spNMix", {
  fitted.out <- fitted(out)
  expect_equal(length(fitted.out), 2)
})

# Check predictions -------------------
test_that("predict works for spNMix", {
  pred.out <- predict(out, X.0, coords.0, verbose = FALSE)
  expect_type(pred.out, "list")
  expect_equal(dim(pred.out$mu.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
  expect_equal(dim(pred.out$N.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
})
test_that("detection prediction works", {
  J.str <- 100
  X.p.0 <- matrix(1, nrow = J.str, ncol = p.det)
  pred.out <- predict(out, X.p.0, type = 'detection')
  expect_type(pred.out, 'list')
  expect_equal(dim(pred.out$p.0.samples), c(out$n.post * out$n.chains, J.str))
})

# Check PPCs --------------------------
test_that("posterior predictive checks work for spNMix", {
  n.post.samples <- out$n.post * out$n.chains
  ppc.out <- ppcAbund(out, 'chi-square', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))
  
  ppc.out <- ppcAbund(out, 'chi-square', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))

  ppc.out <- ppcAbund(out, 'chi-square', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))
})

# Detection covariate only ------------------------------------------------
set.seed(1000)
J.x <- 15
J.y <- 15
J <- J.x * J.y
n.rep <- sample(2:4, J, replace = TRUE)
beta <- c(0)
p.abund <- length(beta)
alpha <- c(0.5, -0.2, 0.3)
p.det <- length(alpha)
mu.RE <- list()
p.RE <- list()
phi <- runif(1, 3 / 1, 3 / .1)
sigma.sq <- 0.5
kappa <- 0.5
phi <- 3 / 0.5
nu <- 1
sp <- TRUE
cov.model <- 'matern'
family <- 'NB'
dat <- simNMix(J.x = J.x, J.y = J.y, n.rep = n.rep, beta = beta, alpha = alpha,
	       mu.RE = mu.RE, p.RE = p.RE, sp = TRUE, family = family, kappa = kappa, 
               sigma.sq = sigma.sq, phi = phi, nu = nu, cov.model = cov.model)

pred.indx <- sample(1:J, round(J * .25), replace = FALSE)
y <- dat$y[-pred.indx, , drop = FALSE]
y.0 <- dat$y[pred.indx, , drop = FALSE]
# Occupancy covariates
X <- dat$X[-pred.indx, , drop = FALSE]
# X.re <- dat$X.re[-pred.indx, , drop = FALSE]
# Prediction covariates
X.0 <- dat$X[pred.indx, , drop = FALSE]
# X.re.0 <- dat$X.re[pred.indx, , drop = FALSE]
# Detection covariates
X.p <- dat$X.p[-pred.indx, , , drop = FALSE]
# X.p.re <- dat$X.p.re[-pred.indx, , , drop = FALSE]
coords <- as.matrix(dat$coords[-pred.indx, ])
coords.0 <- as.matrix(dat$coords[pred.indx, ])

abund.covs <- X
colnames(abund.covs) <- c('int')
det.covs <- list(det.cov.1 = X.p[, , 2], 
                 det.cov.2 = X.p[, , 3])
data.list <- list(y = y,
		  abund.covs = abund.covs,
 		  det.covs = det.covs, 
                  coords = coords)
# Priors
prior.list <- list(beta.normal = list(mean = 0, var = 10),
		   alpha.normal = list(mean = 0, var = 2.72),
                   sigma.sq.ig = c(2, 1),
                   phi.unif = c(3 / 1, 3 / .1), 
                   nu.unif = c(0.1, 5)) 

# Starting values
inits.list <- list(alpha = 0,
		   beta = 0,
		   kappa = 1.3,
		   sigma.sq.mu = 0.5,
		   N = apply(y, 1, max, na.rm = TRUE), 
                   phi = 3 / .5, sigma.sq = 0.5, nu = 2)
tuning <- list(beta = 0.1, alpha = 0.1, beta.star = 0.1, alpha.star = 0.1,
               kappa = 0.2, phi = 0.5, sigma.sq = 0.5, nu = 0.5, w = 1)
n.batch <- 10
batch.length <- 25
n.burn <- 50
n.thin <- 2
n.chains <- 2
abund.formula <- ~ 1
det.formula <- ~ det.cov.1 + det.cov.2


out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      inits = inits.list,
	      family = 'NB',
	      tuning = tuning,
	      priors = prior.list,
	      cov.model = 'matern', 
	      NNGP = TRUE, 
	      n.neighbors = 5,
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 1,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains,
              k.fold = 2,
              k.fold.threads = 1)

# Test to make sure it worked ---------
test_that("out is of class spNMix", {
  expect_s3_class(out, "spNMix")
})

# Check cross-validation --------------
test_that("cross-validation works", {
  expect_equal(length(out$rmspe), 1)
  expect_type(out$rmspe, "double")
  expect_gt(out$rmspe, 0)
})

# Check random effects ----------------
test_that("random effects are empty", {
  expect_equal(out$pRE, FALSE)
  expect_equal(out$muRE, FALSE)
})

# Check output data output is correct -
test_that("out$y == y", {
  expect_equal(out$y, y)
})

# Check Poisson -----------------------
test_that("Poisson works", {
  out <- spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'spherical',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = FALSE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1)
  expect_s3_class(out, "spNMix")
})

# Check default priors ----------------
test_that("default priors, multiple chains, inits, burn, thin work", {
  out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      family = 'NB',
	      NNGP = TRUE,
	      n.neighbors = 5,
	      cov.model = 'exponential',
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 50,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains)
  expect_s3_class(out, 'spNMix')
})

# Check summary -----------------------
test_that("summary works", {
  expect_output(summary(out))
})

# Check verbose -----------------------
test_that("verbose prints to the screen", {
  expect_output(spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'gaussian',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = TRUE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1))
})

# Check waicAbund -----------------------
test_that("waicAbund works for spNMix", {
  # as.vector gets rid of names
  waic.out <- as.vector(waicAbund(out, max(out$N.samples) + 10))
  expect_equal(length(waic.out), 3)
  expect_equal(waic.out[3], -2 * (waic.out[1] - waic.out[2]))
})

# Check fitted ------------------------
test_that("fitted works for spNMix", {
  fitted.out <- fitted(out)
  expect_equal(length(fitted.out), 2)
})

# Check predictions -------------------
test_that("predict works for spNMix", {
  pred.out <- predict(out, X.0, coords.0, verbose = FALSE)
  expect_type(pred.out, "list")
  expect_equal(dim(pred.out$mu.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
  expect_equal(dim(pred.out$N.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
})
test_that("detection prediction works", {
  X.p.0 <- dat$X.p[, 1, ]
  pred.out <- predict(out, X.p.0, type = 'detection')
  expect_type(pred.out, 'list')
  expect_equal(dim(pred.out$p.0.samples), c(out$n.post * out$n.chains, J))
})

# Check PPCs --------------------------
test_that("posterior predictive checks work for spNMix", {
  n.post.samples <- out$n.post * out$n.chains
  ppc.out <- ppcAbund(out, 'chi-square', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))
  
  ppc.out <- ppcAbund(out, 'chi-square', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))

  ppc.out <- ppcAbund(out, 'chi-square', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))
})

# Covariates on both ------------------------------------------------------
set.seed(1000)
J.x <- 15
J.y <- 15
J <- J.x * J.y
n.rep <- sample(2:4, J, replace = TRUE)
beta <- c(0, 0.3, 0.2)
p.abund <- length(beta)
alpha <- c(0.5, -0.2, 0.3)
p.det <- length(alpha)
mu.RE <- list()
p.RE <- list()
phi <- runif(1, 3 / 1, 3 / .1)
sigma.sq <- 0.5
kappa <- 0.5
phi <- 3 / 0.5
nu <- 1
sp <- TRUE
cov.model <- 'matern'
family <- 'NB'
dat <- simNMix(J.x = J.x, J.y = J.y, n.rep = n.rep, beta = beta, alpha = alpha,
	       mu.RE = mu.RE, p.RE = p.RE, sp = TRUE, family = family, kappa = kappa, 
               sigma.sq = sigma.sq, phi = phi, nu = nu, cov.model = cov.model)

pred.indx <- sample(1:J, round(J * .25), replace = FALSE)
y <- dat$y[-pred.indx, , drop = FALSE]
y.0 <- dat$y[pred.indx, , drop = FALSE]
# Occupancy covariates
X <- dat$X[-pred.indx, , drop = FALSE]
# X.re <- dat$X.re[-pred.indx, , drop = FALSE]
# Prediction covariates
X.0 <- dat$X[pred.indx, , drop = FALSE]
# X.re.0 <- dat$X.re[pred.indx, , drop = FALSE]
# Detection covariates
X.p <- dat$X.p[-pred.indx, , , drop = FALSE]
# X.p.re <- dat$X.p.re[-pred.indx, , , drop = FALSE]
coords <- as.matrix(dat$coords[-pred.indx, ])
coords.0 <- as.matrix(dat$coords[pred.indx, ])

abund.covs <- X
colnames(abund.covs) <- c('int', 'occ.cov.1', 'occ.cov.2')
det.covs <- list(det.cov.1 = X.p[, , 2], 
                 det.cov.2 = X.p[, , 3])
data.list <- list(y = y,
		  abund.covs = abund.covs,
 		  det.covs = det.covs, 
                  coords = coords)
# Priors
prior.list <- list(beta.normal = list(mean = 0, var = 10),
		   alpha.normal = list(mean = 0, var = 2.72),
                   sigma.sq.ig = c(2, 1),
                   phi.unif = c(3 / 1, 3 / .1), 
                   nu.unif = c(0.1, 5)) 

# Starting values
inits.list <- list(alpha = 0,
		   beta = 0,
		   kappa = 1.3,
		   sigma.sq.mu = 0.5,
		   N = apply(y, 1, max, na.rm = TRUE), 
                   phi = 3 / .5, sigma.sq = 0.5, nu = 2)
tuning <- list(beta = 0.1, alpha = 0.1, beta.star = 0.1, alpha.star = 0.1,
               kappa = 0.2, phi = 0.5, sigma.sq = 0.5, nu = 0.5, w = 1)
n.batch <- 10
batch.length <- 25
n.burn <- 50
n.thin <- 2
n.chains <- 2
abund.formula <- ~ occ.cov.1 + occ.cov.2
det.formula <- ~ det.cov.1 + det.cov.2


out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      inits = inits.list,
	      family = 'NB',
	      tuning = tuning,
	      priors = prior.list,
	      cov.model = 'matern', 
	      NNGP = TRUE, 
	      n.neighbors = 5,
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 1,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains,
              k.fold = 2,
              k.fold.threads = 1)

# Test to make sure it worked ---------
test_that("out is of class spNMix", {
  expect_s3_class(out, "spNMix")
})

# Check cross-validation --------------
test_that("cross-validation works", {
  expect_equal(length(out$rmspe), 1)
  expect_type(out$rmspe, "double")
  expect_gt(out$rmspe, 0)
})

# Check random effects ----------------
test_that("random effects are empty", {
  expect_equal(out$pRE, FALSE)
  expect_equal(out$muRE, FALSE)
})

# Check output data output is correct -
test_that("out$y == y", {
  expect_equal(out$y, y)
})

# Check Poisson -----------------------
test_that("Poisson works", {
  out <- spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'spherical',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = FALSE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1)
  expect_s3_class(out, "spNMix")
})

# Check default priors ----------------
test_that("default priors, multiple chains, inits, burn, thin work", {
  out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      family = 'NB',
	      NNGP = TRUE,
	      n.neighbors = 5,
	      cov.model = 'exponential',
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 50,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains)
  expect_s3_class(out, 'spNMix')
})

# Check summary -----------------------
test_that("summary works", {
  expect_output(summary(out))
})

# Check verbose -----------------------
test_that("verbose prints to the screen", {
  expect_output(spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'gaussian',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = TRUE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1))
})

# Check waicAbund -----------------------
test_that("waicAbund works for spNMix", {
  # as.vector gets rid of names
  waic.out <- as.vector(waicAbund(out, max(out$N.samples) + 10))
  expect_equal(length(waic.out), 3)
  expect_equal(waic.out[3], -2 * (waic.out[1] - waic.out[2]))
})

# Check fitted ------------------------
test_that("fitted works for spNMix", {
  fitted.out <- fitted(out)
  expect_equal(length(fitted.out), 2)
})

# Check predictions -------------------
test_that("predict works for spNMix", {
  pred.out <- predict(out, X.0, coords.0, verbose = FALSE)
  expect_type(pred.out, "list")
  expect_equal(dim(pred.out$mu.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
  expect_equal(dim(pred.out$N.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
})
test_that("detection prediction works", {
  X.p.0 <- dat$X.p[, 1, ]
  pred.out <- predict(out, X.p.0, type = 'detection')
  expect_type(pred.out, 'list')
  expect_equal(dim(pred.out$p.0.samples), c(out$n.post * out$n.chains, J))
})

# Check PPCs --------------------------
test_that("posterior predictive checks work for spNMix", {
  n.post.samples <- out$n.post * out$n.chains
  ppc.out <- ppcAbund(out, 'chi-square', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))
  
  ppc.out <- ppcAbund(out, 'chi-square', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))

  ppc.out <- ppcAbund(out, 'chi-square', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))
})

# Interactions on both ----------------------------------------------------
set.seed(1000)
J.x <- 15
J.y <- 15
J <- J.x * J.y
n.rep <- sample(2:4, J, replace = TRUE)
beta <- c(0, 0.3, 0.2)
p.abund <- length(beta)
alpha <- c(0.5, -0.2, 0.3)
p.det <- length(alpha)
mu.RE <- list()
p.RE <- list()
phi <- runif(1, 3 / 1, 3 / .1)
sigma.sq <- 0.5
kappa <- 0.5
phi <- 3 / 0.5
nu <- 1
sp <- TRUE
cov.model <- 'matern'
family <- 'NB'
dat <- simNMix(J.x = J.x, J.y = J.y, n.rep = n.rep, beta = beta, alpha = alpha,
	       mu.RE = mu.RE, p.RE = p.RE, sp = TRUE, family = family, kappa = kappa, 
               sigma.sq = sigma.sq, phi = phi, nu = nu, cov.model = cov.model)

pred.indx <- sample(1:J, round(J * .25), replace = FALSE)
y <- dat$y[-pred.indx, , drop = FALSE]
y.0 <- dat$y[pred.indx, , drop = FALSE]
# Occupancy covariates
X <- dat$X[-pred.indx, , drop = FALSE]
# X.re <- dat$X.re[-pred.indx, , drop = FALSE]
# Prediction covariates
X.0 <- dat$X[pred.indx, , drop = FALSE]
# X.re.0 <- dat$X.re[pred.indx, , drop = FALSE]
# Detection covariates
X.p <- dat$X.p[-pred.indx, , , drop = FALSE]
# X.p.re <- dat$X.p.re[-pred.indx, , , drop = FALSE]
coords <- as.matrix(dat$coords[-pred.indx, ])
coords.0 <- as.matrix(dat$coords[pred.indx, ])

abund.covs <- X
colnames(abund.covs) <- c('int', 'occ.cov.1', 'occ.cov.2')
det.covs <- list(det.cov.1 = X.p[, , 2], 
                 det.cov.2 = X.p[, , 3])
data.list <- list(y = y,
		  abund.covs = abund.covs,
 		  det.covs = det.covs, 
                  coords = coords)
# Priors
prior.list <- list(beta.normal = list(mean = 0, var = 10),
		   alpha.normal = list(mean = 0, var = 2.72),
                   sigma.sq.ig = c(2, 1),
                   phi.unif = c(3 / 1, 3 / .1), 
                   nu.unif = c(0.1, 5)) 

# Starting values
inits.list <- list(alpha = 0,
		   beta = 0,
		   kappa = 1.3,
		   sigma.sq.mu = 0.5,
		   N = apply(y, 1, max, na.rm = TRUE), 
                   phi = 3 / .5, sigma.sq = 0.5, nu = 2)
tuning <- list(beta = 0.1, alpha = 0.1, beta.star = 0.1, alpha.star = 0.1,
               kappa = 0.2, phi = 0.5, sigma.sq = 0.5, nu = 0.5, w = 1)
n.batch <- 10
batch.length <- 25
n.burn <- 50
n.thin <- 2
n.chains <- 2
abund.formula <- ~ occ.cov.1 * occ.cov.2
det.formula <- ~ det.cov.1 * det.cov.2


out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      inits = inits.list,
	      family = 'NB',
	      tuning = tuning,
	      priors = prior.list,
	      cov.model = 'matern', 
	      NNGP = TRUE, 
	      n.neighbors = 5,
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 1,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains,
              k.fold = 2,
              k.fold.threads = 1)

# Test to make sure it worked ---------
test_that("out is of class spNMix", {
  expect_s3_class(out, "spNMix")
})

# Check cross-validation --------------
test_that("cross-validation works", {
  expect_equal(length(out$rmspe), 1)
  expect_type(out$rmspe, "double")
  expect_gt(out$rmspe, 0)
})

# Check random effects ----------------
test_that("random effects are empty", {
  expect_equal(out$pRE, FALSE)
  expect_equal(out$muRE, FALSE)
})

# Check output data output is correct -
test_that("out$y == y", {
  expect_equal(out$y, y)
})

# Check Poisson -----------------------
test_that("Poisson works", {
  out <- spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'spherical',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = FALSE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1)
  expect_s3_class(out, "spNMix")
})

# Check default priors ----------------
test_that("default priors, multiple chains, inits, burn, thin work", {
  out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      family = 'NB',
	      NNGP = TRUE,
	      n.neighbors = 5,
	      cov.model = 'exponential',
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 50,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains)
  expect_s3_class(out, 'spNMix')
})

# Check summary -----------------------
test_that("summary works", {
  expect_output(summary(out))
})

# Check verbose -----------------------
test_that("verbose prints to the screen", {
  expect_output(spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'gaussian',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = TRUE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1))
})

# Check waicAbund -----------------------
test_that("waicAbund works for spNMix", {
  # as.vector gets rid of names
  waic.out <- as.vector(waicAbund(out, max(out$N.samples) + 10))
  expect_equal(length(waic.out), 3)
  expect_equal(waic.out[3], -2 * (waic.out[1] - waic.out[2]))
})

# Check fitted ------------------------
test_that("fitted works for spNMix", {
  fitted.out <- fitted(out)
  expect_equal(length(fitted.out), 2)
})

# Check predictions -------------------
test_that("predict works for spNMix", {
  X.0 <- cbind(X.0, X.0[, 2] * X.0[, 3])
  pred.out <- predict(out, X.0, coords.0, verbose = FALSE)
  expect_type(pred.out, "list")
  expect_equal(dim(pred.out$mu.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
  expect_equal(dim(pred.out$N.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
})
test_that("detection prediction works", {
  X.p.0 <- dat$X.p[, 1, ]
  X.p.0 <- cbind(X.p.0, X.p.0[, 2] * X.p.0[, 3])
  pred.out <- predict(out, X.p.0, type = 'detection')
  expect_type(pred.out, 'list')
  expect_equal(dim(pred.out$p.0.samples), c(out$n.post * out$n.chains, J))
})

# Check PPCs --------------------------
test_that("posterior predictive checks work for spNMix", {
  n.post.samples <- out$n.post * out$n.chains
  ppc.out <- ppcAbund(out, 'chi-square', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))
  
  ppc.out <- ppcAbund(out, 'chi-square', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))

  ppc.out <- ppcAbund(out, 'chi-square', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))
})

# Site covariate on detection ---------------------------------------------
set.seed(1000)
J.x <- 15
J.y <- 15
J <- J.x * J.y
n.rep <- sample(2:4, J, replace = TRUE)
beta <- c(0, 0.3, 0.2)
p.abund <- length(beta)
alpha <- c(0.5, -0.2, 0.3)
p.det <- length(alpha)
mu.RE <- list()
p.RE <- list()
phi <- runif(1, 3 / 1, 3 / .1)
sigma.sq <- 0.5
kappa <- 0.5
phi <- 3 / 0.5
nu <- 1
sp <- TRUE
cov.model <- 'matern'
family <- 'NB'
dat <- simNMix(J.x = J.x, J.y = J.y, n.rep = n.rep, beta = beta, alpha = alpha,
	       mu.RE = mu.RE, p.RE = p.RE, sp = TRUE, family = family, kappa = kappa, 
               sigma.sq = sigma.sq, phi = phi, nu = nu, cov.model = cov.model)

pred.indx <- sample(1:J, round(J * .25), replace = FALSE)
y <- dat$y[-pred.indx, , drop = FALSE]
y.0 <- dat$y[pred.indx, , drop = FALSE]
# Occupancy covariates
X <- dat$X[-pred.indx, , drop = FALSE]
# X.re <- dat$X.re[-pred.indx, , drop = FALSE]
# Prediction covariates
X.0 <- dat$X[pred.indx, , drop = FALSE]
# X.re.0 <- dat$X.re[pred.indx, , drop = FALSE]
# Detection covariates
X.p <- dat$X.p[-pred.indx, , , drop = FALSE]
# X.p.re <- dat$X.p.re[-pred.indx, , , drop = FALSE]
coords <- as.matrix(dat$coords[-pred.indx, ])
coords.0 <- as.matrix(dat$coords[pred.indx, ])

abund.covs <- X
colnames(abund.covs) <- c('int', 'occ.cov.1', 'occ.cov.2')
det.covs <- list(det.cov.1 = X[, 2])
data.list <- list(y = y,
		  abund.covs = abund.covs,
 		  det.covs = det.covs, 
                  coords = coords)
# Priors
prior.list <- list(beta.normal = list(mean = 0, var = 10),
		   alpha.normal = list(mean = 0, var = 2.72),
                   sigma.sq.ig = c(2, 1),
                   phi.unif = c(3 / 1, 3 / .1), 
                   nu.unif = c(0.1, 5)) 

# Starting values
inits.list <- list(alpha = 0,
		   beta = 0,
		   kappa = 1.3,
		   sigma.sq.mu = 0.5,
		   N = apply(y, 1, max, na.rm = TRUE), 
                   phi = 3 / .5, sigma.sq = 0.5, nu = 2)
tuning <- list(beta = 0.1, alpha = 0.1, beta.star = 0.1, alpha.star = 0.1,
               kappa = 0.2, phi = 0.5, sigma.sq = 0.5, nu = 0.5, w = 1)
n.batch <- 10
batch.length <- 25
n.burn <- 50
n.thin <- 2
n.chains <- 2
abund.formula <- ~ occ.cov.1 + occ.cov.2
det.formula <- ~ det.cov.1


out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      inits = inits.list,
	      family = 'NB',
	      tuning = tuning,
	      priors = prior.list,
	      cov.model = 'matern', 
	      NNGP = TRUE, 
	      n.neighbors = 5,
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 1,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains,
              k.fold = 2,
              k.fold.threads = 1)

# Test to make sure it worked ---------
test_that("out is of class spNMix", {
  expect_s3_class(out, "spNMix")
})

# Check cross-validation --------------
test_that("cross-validation works", {
  expect_equal(length(out$rmspe), 1)
  expect_type(out$rmspe, "double")
  expect_gt(out$rmspe, 0)
})

# Check random effects ----------------
test_that("random effects are empty", {
  expect_equal(out$pRE, FALSE)
  expect_equal(out$muRE, FALSE)
})

# Check output data output is correct -
test_that("out$y == y", {
  expect_equal(out$y, y)
})

# Check Poisson -----------------------
test_that("Poisson works", {
  out <- spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'spherical',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = FALSE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1)
  expect_s3_class(out, "spNMix")
})

# Check default priors ----------------
test_that("default priors, multiple chains, inits, burn, thin work", {
  out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      family = 'NB',
	      NNGP = TRUE,
	      n.neighbors = 5,
	      cov.model = 'exponential',
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 50,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains)
  expect_s3_class(out, 'spNMix')
})

# Check summary -----------------------
test_that("summary works", {
  expect_output(summary(out))
})

# Check verbose -----------------------
test_that("verbose prints to the screen", {
  expect_output(spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'gaussian',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = TRUE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1))
})

# Check waicAbund -----------------------
test_that("waicAbund works for spNMix", {
  # as.vector gets rid of names
  waic.out <- as.vector(waicAbund(out, max(out$N.samples) + 10))
  expect_equal(length(waic.out), 3)
  expect_equal(waic.out[3], -2 * (waic.out[1] - waic.out[2]))
})

# Check fitted ------------------------
test_that("fitted works for spNMix", {
  fitted.out <- fitted(out)
  expect_equal(length(fitted.out), 2)
})

# Check predictions -------------------
test_that("predict works for spNMix", {
  pred.out <- predict(out, X.0, coords.0, verbose = FALSE)
  expect_type(pred.out, "list")
  expect_equal(dim(pred.out$mu.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
  expect_equal(dim(pred.out$N.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
})
test_that("detection prediction works", {
  X.p.0 <- dat$X.p[, 1, 1:2]
  pred.out <- predict(out, X.p.0, type = 'detection')
  expect_type(pred.out, 'list')
  expect_equal(dim(pred.out$p.0.samples), c(out$n.post * out$n.chains, J))
})

# Check PPCs --------------------------
test_that("posterior predictive checks work for spNMix", {
  n.post.samples <- out$n.post * out$n.chains
  ppc.out <- ppcAbund(out, 'chi-square', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))
  
  ppc.out <- ppcAbund(out, 'chi-square', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))

  ppc.out <- ppcAbund(out, 'chi-square', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))
})

# Random intercept on abundance -------------------------------------------
set.seed(1000)
J.x <- 15
J.y <- 15
J <- J.x * J.y
n.rep <- sample(2:4, J, replace = TRUE)
beta <- c(0)
p.abund <- length(beta)
alpha <- c(0.5)
p.det <- length(alpha)
mu.RE <- list(levels = c(15),
	       sigma.sq.mu = c(1.3))
p.RE <- list()
phi <- runif(1, 3 / 1, 3 / .1)
sigma.sq <- 0.5
kappa <- 0.5
phi <- 3 / 0.5
nu <- 1
sp <- TRUE
cov.model <- 'matern'
family <- 'NB'
dat <- simNMix(J.x = J.x, J.y = J.y, n.rep = n.rep, beta = beta, alpha = alpha,
	       mu.RE = mu.RE, p.RE = p.RE, sp = TRUE, family = family, kappa = kappa, 
               sigma.sq = sigma.sq, phi = phi, nu = nu, cov.model = cov.model)

pred.indx <- sample(1:J, round(J * .25), replace = FALSE)
y <- dat$y[-pred.indx, , drop = FALSE]
y.0 <- dat$y[pred.indx, , drop = FALSE]
# Occupancy covariates
X <- dat$X[-pred.indx, , drop = FALSE]
X.re <- dat$X.re[-pred.indx, , drop = FALSE]
# Prediction covariates
X.0 <- dat$X[pred.indx, , drop = FALSE]
X.re.0 <- dat$X.re[pred.indx, , drop = FALSE]
# Detection covariates
X.p <- dat$X.p[-pred.indx, , , drop = FALSE]
# X.p.re <- dat$X.p.re[-pred.indx, , , drop = FALSE]
coords <- as.matrix(dat$coords[-pred.indx, ])
coords.0 <- as.matrix(dat$coords[pred.indx, ])

abund.covs <- cbind(X, X.re)
colnames(abund.covs) <- c('int', 'abund.factor.1')
det.covs <- list(int = X.p[, , 1])
data.list <- list(y = y,
		  abund.covs = abund.covs,
 		  det.covs = det.covs, 
                  coords = coords)
# Priors
prior.list <- list(beta.normal = list(mean = 0, var = 10),
		   alpha.normal = list(mean = 0, var = 2.72),
                   sigma.sq.ig = c(2, 1),
                   phi.unif = c(3 / 1, 3 / .1), 
                   nu.unif = c(0.1, 5)) 

# Starting values
inits.list <- list(alpha = 0,
		   beta = 0,
		   kappa = 1.3,
		   sigma.sq.mu = 0.5,
		   N = apply(y, 1, max, na.rm = TRUE), 
                   phi = 3 / .5, sigma.sq = 0.5, nu = 2)
tuning <- list(beta = 0.1, alpha = 0.1, beta.star = 0.1, alpha.star = 0.1,
               kappa = 0.2, phi = 0.5, sigma.sq = 0.5, nu = 0.5, w = 1)
n.batch <- 10
batch.length <- 25
n.burn <- 50
n.thin <- 2
n.chains <- 2
abund.formula <- ~ (1 | abund.factor.1)
det.formula <- ~ 1

out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      inits = inits.list,
	      family = 'NB',
	      tuning = tuning,
	      priors = prior.list,
	      cov.model = 'matern', 
	      NNGP = TRUE, 
	      n.neighbors = 5,
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 1,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains,
              k.fold = 2,
              k.fold.threads = 1)

# Test to make sure it worked ---------
test_that("out is of class spNMix", {
  expect_s3_class(out, "spNMix")
})

# Check cross-validation --------------
test_that("cross-validation works", {
  expect_equal(length(out$rmspe), 1)
  expect_type(out$rmspe, "double")
  expect_gt(out$rmspe, 0)
})

# Check random effects ----------------
test_that("random effects are empty", {
  expect_equal(out$pRE, FALSE)
  expect_equal(out$muRE, TRUE)
})

# Check output data output is correct -
test_that("out$y == y", {
  expect_equal(out$y, y)
})

test_that("random effect gives error when non-numeric", {
  data.list$abund.covs <- as.data.frame(data.list$abund.covs)
  data.list$abund.covs$abund.factor.1 <- factor(data.list$abund.covs$abund.factor.1)
  expect_error(out <- spNMix(abund.formula = abund.formula,
	    det.formula = det.formula,
	    data = data.list,
	    n.batch = n.batch,
	    batch.length = batch.length,
	    inits = inits.list,
	    family = 'NB',
	    tuning = tuning,
	    priors = prior.list,
	    accept.rate = 0.43,
	    n.omp.threads = 1,
	    verbose = FALSE,
	    n.report = 50,
	    n.burn = n.burn,
	    n.thin = n.thin,
	    n.chains = n.chains))
  data.list$abund.covs$abund.factor.1 <- factor(data.list$abund.covs$abund.factor.1)
  expect_error(out <- spNMix(abund.formula = abund.formula,
	    det.formula = det.formula,
	    data = data.list,
	    n.batch = n.batch,
	    batch.length = batch.length,
	    inits = inits.list,
	    family = 'NB',
	    tuning = tuning,
	    priors = prior.list,
	    accept.rate = 0.43,
	    n.omp.threads = 1,
	    verbose = FALSE,
	    n.report = 50,
	    n.burn = n.burn,
	    n.thin = n.thin,
	    n.chains = n.chains))
})

# Check Poisson -----------------------
test_that("Poisson works", {
  out <- spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'spherical',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = FALSE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1)
  expect_s3_class(out, "spNMix")
})

# Check default priors ----------------
test_that("default priors, multiple chains, inits, burn, thin work", {
  out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      family = 'NB',
	      NNGP = TRUE,
	      n.neighbors = 5,
	      cov.model = 'exponential',
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 50,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains)
  expect_s3_class(out, 'spNMix')
})

# Check summary -----------------------
test_that("summary works", {
  expect_output(summary(out))
})

# Check verbose -----------------------
test_that("verbose prints to the screen", {
  expect_output(spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'gaussian',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = TRUE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1))
})

# Check waicAbund -----------------------
test_that("waicAbund works for spNMix", {
  # as.vector gets rid of names
  waic.out <- as.vector(waicAbund(out, max(out$N.samples) + 10))
  expect_equal(length(waic.out), 3)
  expect_equal(waic.out[3], -2 * (waic.out[1] - waic.out[2]))
})

# Check fitted ------------------------
test_that("fitted works for spNMix", {
  fitted.out <- fitted(out)
  expect_equal(length(fitted.out), 2)
})

# Check predictions -------------------
test_that("predict works for spNMix", {
  X.0 <- cbind(X.0, X.re.0)
  colnames(X.0) <- c('(Intercept)', 'abund.factor.1')
  pred.out <- predict(out, X.0, coords.0, verbose = FALSE)
  expect_type(pred.out, "list")
  expect_equal(dim(pred.out$mu.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
  expect_equal(dim(pred.out$N.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
})
test_that("detection prediction works", {
  J.str <- 100
  X.p.0 <- matrix(1, nrow = J.str, ncol = p.det)
  pred.out <- predict(out, X.p.0, type = 'detection')
  expect_type(pred.out, 'list')
  expect_equal(dim(pred.out$p.0.samples), c(out$n.post * out$n.chains, J.str))
})

# Check PPCs --------------------------
test_that("posterior predictive checks work for spNMix", {
  n.post.samples <- out$n.post * out$n.chains
  ppc.out <- ppcAbund(out, 'chi-square', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))
  
  ppc.out <- ppcAbund(out, 'chi-square', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))

  ppc.out <- ppcAbund(out, 'chi-square', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))
})

# Multiple random intercepts on abundance ---------------------------------
set.seed(1000)
J.x <- 15
J.y <- 15
J <- J.x * J.y
n.rep <- sample(2:4, J, replace = TRUE)
beta <- c(0)
p.abund <- length(beta)
alpha <- c(0.5)
p.det <- length(alpha)
mu.RE <- list(levels = c(15, 10),
	       sigma.sq.mu = c(1.3, 0.3))
p.RE <- list()
phi <- runif(1, 3 / 1, 3 / .1)
sigma.sq <- 0.5
kappa <- 0.5
phi <- 3 / 0.5
nu <- 1
sp <- TRUE
cov.model <- 'matern'
family <- 'NB'
dat <- simNMix(J.x = J.x, J.y = J.y, n.rep = n.rep, beta = beta, alpha = alpha,
	       mu.RE = mu.RE, p.RE = p.RE, sp = TRUE, family = family, kappa = kappa, 
               sigma.sq = sigma.sq, phi = phi, nu = nu, cov.model = cov.model)

pred.indx <- sample(1:J, round(J * .25), replace = FALSE)
y <- dat$y[-pred.indx, , drop = FALSE]
y.0 <- dat$y[pred.indx, , drop = FALSE]
# Occupancy covariates
X <- dat$X[-pred.indx, , drop = FALSE]
X.re <- dat$X.re[-pred.indx, , drop = FALSE]
# Prediction covariates
X.0 <- dat$X[pred.indx, , drop = FALSE]
X.re.0 <- dat$X.re[pred.indx, , drop = FALSE]
# Detection covariates
X.p <- dat$X.p[-pred.indx, , , drop = FALSE]
# X.p.re <- dat$X.p.re[-pred.indx, , , drop = FALSE]
coords <- as.matrix(dat$coords[-pred.indx, ])
coords.0 <- as.matrix(dat$coords[pred.indx, ])

abund.covs <- cbind(X, X.re)
colnames(abund.covs) <- c('int', 'abund.factor.1', 'abund.factor.2')
det.covs <- list(int = X.p[, , 1])
data.list <- list(y = y,
		  abund.covs = abund.covs,
 		  det.covs = det.covs, 
                  coords = coords)
# Priors
prior.list <- list(beta.normal = list(mean = 0, var = 10),
		   alpha.normal = list(mean = 0, var = 2.72),
                   sigma.sq.ig = c(2, 1),
                   phi.unif = c(3 / 1, 3 / .1), 
                   nu.unif = c(0.1, 5)) 

# Starting values
inits.list <- list(alpha = 0,
		   beta = 0,
		   kappa = 1.3,
		   sigma.sq.mu = 0.5,
		   N = apply(y, 1, max, na.rm = TRUE), 
                   phi = 3 / .5, sigma.sq = 0.5, nu = 2)
tuning <- list(beta = 0.1, alpha = 0.1, beta.star = 0.1, alpha.star = 0.1,
               kappa = 0.2, phi = 0.5, sigma.sq = 0.5, nu = 0.5, w = 1)
n.batch <- 10
batch.length <- 25
n.burn <- 50
n.thin <- 2
n.chains <- 2
abund.formula <- ~ (1 | abund.factor.1) + (1 | abund.factor.2)
det.formula <- ~ 1

out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      inits = inits.list,
	      family = 'NB',
	      tuning = tuning,
	      priors = prior.list,
	      cov.model = 'matern', 
	      NNGP = TRUE, 
	      n.neighbors = 5,
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 1,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains,
              k.fold = 2,
              k.fold.threads = 1)

# Test to make sure it worked ---------
test_that("out is of class spNMix", {
  expect_s3_class(out, "spNMix")
})

# Check cross-validation --------------
test_that("cross-validation works", {
  expect_equal(length(out$rmspe), 1)
  expect_type(out$rmspe, "double")
  expect_gt(out$rmspe, 0)
})

# Check random effects ----------------
test_that("random effects are empty", {
  expect_equal(out$pRE, FALSE)
  expect_equal(out$muRE, TRUE)
})

# Check output data output is correct -
test_that("out$y == y", {
  expect_equal(out$y, y)
})

test_that("random effect gives error when non-numeric", {
  data.list$abund.covs <- as.data.frame(data.list$abund.covs)
  data.list$abund.covs$abund.factor.1 <- factor(data.list$abund.covs$abund.factor.1)
  expect_error(out <- spNMix(abund.formula = abund.formula,
	    det.formula = det.formula,
	    data = data.list,
	    n.batch = n.batch,
	    batch.length = batch.length,
	    inits = inits.list,
	    family = 'NB',
	    tuning = tuning,
	    priors = prior.list,
	    accept.rate = 0.43,
	    n.omp.threads = 1,
	    verbose = FALSE,
	    n.report = 50,
	    n.burn = n.burn,
	    n.thin = n.thin,
	    n.chains = n.chains))
  data.list$abund.covs$abund.factor.1 <- factor(data.list$abund.covs$abund.factor.1)
  expect_error(out <- spNMix(abund.formula = abund.formula,
	    det.formula = det.formula,
	    data = data.list,
	    n.batch = n.batch,
	    batch.length = batch.length,
	    inits = inits.list,
	    family = 'NB',
	    tuning = tuning,
	    priors = prior.list,
	    accept.rate = 0.43,
	    n.omp.threads = 1,
	    verbose = FALSE,
	    n.report = 50,
	    n.burn = n.burn,
	    n.thin = n.thin,
	    n.chains = n.chains))
})

# Check Poisson -----------------------
test_that("Poisson works", {
  out <- spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'spherical',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = FALSE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1)
  expect_s3_class(out, "spNMix")
})

# Check default priors ----------------
test_that("default priors, multiple chains, inits, burn, thin work", {
  out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      family = 'NB',
	      NNGP = TRUE,
	      n.neighbors = 5,
	      cov.model = 'exponential',
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 50,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains)
  expect_s3_class(out, 'spNMix')
})

# Check summary -----------------------
test_that("summary works", {
  expect_output(summary(out))
})

# Check verbose -----------------------
test_that("verbose prints to the screen", {
  expect_output(spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'gaussian',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = TRUE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1))
})

# Check waicAbund -----------------------
test_that("waicAbund works for spNMix", {
  # as.vector gets rid of names
  waic.out <- as.vector(waicAbund(out, max(out$N.samples) + 10))
  expect_equal(length(waic.out), 3)
  expect_equal(waic.out[3], -2 * (waic.out[1] - waic.out[2]))
})

# Check fitted ------------------------
test_that("fitted works for spNMix", {
  fitted.out <- fitted(out)
  expect_equal(length(fitted.out), 2)
})

# Check predictions -------------------
test_that("predict works for spNMix", {
  X.0 <- cbind(X.0, X.re.0)
  colnames(X.0) <- c('(Intercept)', 'abund.factor.1', 'abund.factor.2')
  pred.out <- predict(out, X.0, coords.0, verbose = FALSE)
  expect_type(pred.out, "list")
  expect_equal(dim(pred.out$mu.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
  expect_equal(dim(pred.out$N.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
})
test_that("detection prediction works", {
  J.str <- 100
  X.p.0 <- matrix(1, nrow = J.str, ncol = p.det)
  pred.out <- predict(out, X.p.0, type = 'detection')
  expect_type(pred.out, 'list')
  expect_equal(dim(pred.out$p.0.samples), c(out$n.post * out$n.chains, J.str))
})

# Check PPCs --------------------------
test_that("posterior predictive checks work for spNMix", {
  n.post.samples <- out$n.post * out$n.chains
  ppc.out <- ppcAbund(out, 'chi-square', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))
  
  ppc.out <- ppcAbund(out, 'chi-square', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))

  ppc.out <- ppcAbund(out, 'chi-square', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))
})

# Abundance REs + covariates ----------------------------------------------
set.seed(1000)
J.x <- 15
J.y <- 15
J <- J.x * J.y
n.rep <- sample(2:4, J, replace = TRUE)
beta <- c(0, -0.2, 0.2)
p.abund <- length(beta)
alpha <- c(0.5)
p.det <- length(alpha)
mu.RE <- list(levels = c(15, 10),
	       sigma.sq.mu = c(1.3, 0.3))
p.RE <- list()
phi <- runif(1, 3 / 1, 3 / .1)
sigma.sq <- 0.5
kappa <- 0.5
phi <- 3 / 0.5
nu <- 1
sp <- TRUE
cov.model <- 'matern'
family <- 'NB'
dat <- simNMix(J.x = J.x, J.y = J.y, n.rep = n.rep, beta = beta, alpha = alpha,
	       mu.RE = mu.RE, p.RE = p.RE, sp = TRUE, family = family, kappa = kappa, 
               sigma.sq = sigma.sq, phi = phi, nu = nu, cov.model = cov.model)

pred.indx <- sample(1:J, round(J * .25), replace = FALSE)
y <- dat$y[-pred.indx, , drop = FALSE]
y.0 <- dat$y[pred.indx, , drop = FALSE]
# Occupancy covariates
X <- dat$X[-pred.indx, , drop = FALSE]
X.re <- dat$X.re[-pred.indx, , drop = FALSE]
# Prediction covariates
X.0 <- dat$X[pred.indx, , drop = FALSE]
X.re.0 <- dat$X.re[pred.indx, , drop = FALSE]
# Detection covariates
X.p <- dat$X.p[-pred.indx, , , drop = FALSE]
# X.p.re <- dat$X.p.re[-pred.indx, , , drop = FALSE]
coords <- as.matrix(dat$coords[-pred.indx, ])
coords.0 <- as.matrix(dat$coords[pred.indx, ])

abund.covs <- cbind(X, X.re)
colnames(abund.covs) <- c('int', 'abund.cov.1', 'abund.cov.2', 'abund.factor.1', 'abund.factor.2')
det.covs <- list(int = X.p[, , 1])
data.list <- list(y = y,
		  abund.covs = abund.covs,
 		  det.covs = det.covs, 
                  coords = coords)
# Priors
prior.list <- list(beta.normal = list(mean = 0, var = 10),
		   alpha.normal = list(mean = 0, var = 2.72),
                   sigma.sq.ig = c(2, 1),
                   phi.unif = c(3 / 1, 3 / .1), 
                   nu.unif = c(0.1, 5)) 

# Starting values
inits.list <- list(alpha = 0,
		   beta = 0,
		   kappa = 1.3,
		   sigma.sq.mu = 0.5,
		   N = apply(y, 1, max, na.rm = TRUE), 
                   phi = 3 / .5, sigma.sq = 0.5, nu = 2)
tuning <- list(beta = 0.1, alpha = 0.1, beta.star = 0.1, alpha.star = 0.1,
               kappa = 0.2, phi = 0.5, sigma.sq = 0.5, nu = 0.5, w = 1)
n.batch <- 10
batch.length <- 25
n.burn <- 50
n.thin <- 2
n.chains <- 2
abund.formula <- ~ (1 | abund.factor.1) + (1 | abund.factor.2) + abund.cov.1 + abund.cov.2
det.formula <- ~ 1

out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      inits = inits.list,
	      family = 'NB',
	      tuning = tuning,
	      priors = prior.list,
	      cov.model = 'matern', 
	      NNGP = TRUE, 
	      n.neighbors = 5,
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 1,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains,
              k.fold = 2,
              k.fold.threads = 1)

# Test to make sure it worked ---------
test_that("out is of class spNMix", {
  expect_s3_class(out, "spNMix")
})

# Check cross-validation --------------
test_that("cross-validation works", {
  expect_equal(length(out$rmspe), 1)
  expect_type(out$rmspe, "double")
  expect_gt(out$rmspe, 0)
})

# Check random effects ----------------
test_that("random effects are empty", {
  expect_equal(out$pRE, FALSE)
  expect_equal(out$muRE, TRUE)
})

# Check output data output is correct -
test_that("out$y == y", {
  expect_equal(out$y, y)
})

test_that("random effect gives error when non-numeric", {
  data.list$abund.covs <- as.data.frame(data.list$abund.covs)
  data.list$abund.covs$abund.factor.1 <- factor(data.list$abund.covs$abund.factor.1)
  expect_error(out <- spNMix(abund.formula = abund.formula,
	    det.formula = det.formula,
	    data = data.list,
	    n.batch = n.batch,
	    batch.length = batch.length,
	    inits = inits.list,
	    family = 'NB',
	    tuning = tuning,
	    priors = prior.list,
	    accept.rate = 0.43,
	    n.omp.threads = 1,
	    verbose = FALSE,
	    n.report = 50,
	    n.burn = n.burn,
	    n.thin = n.thin,
	    n.chains = n.chains))
  data.list$abund.covs$abund.factor.1 <- factor(data.list$abund.covs$abund.factor.1)
  expect_error(out <- spNMix(abund.formula = abund.formula,
	    det.formula = det.formula,
	    data = data.list,
	    n.batch = n.batch,
	    batch.length = batch.length,
	    inits = inits.list,
	    family = 'NB',
	    tuning = tuning,
	    priors = prior.list,
	    accept.rate = 0.43,
	    n.omp.threads = 1,
	    verbose = FALSE,
	    n.report = 50,
	    n.burn = n.burn,
	    n.thin = n.thin,
	    n.chains = n.chains))
})

# Check Poisson -----------------------
test_that("Poisson works", {
  out <- spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'spherical',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = FALSE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1)
  expect_s3_class(out, "spNMix")
})

# Check default priors ----------------
test_that("default priors, multiple chains, inits, burn, thin work", {
  out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      family = 'NB',
	      NNGP = TRUE,
	      n.neighbors = 5,
	      cov.model = 'exponential',
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 50,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains)
  expect_s3_class(out, 'spNMix')
})

# Check summary -----------------------
test_that("summary works", {
  expect_output(summary(out))
})

# Check verbose -----------------------
test_that("verbose prints to the screen", {
  expect_output(spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'gaussian',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = TRUE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1))
})

# Check waicAbund -----------------------
test_that("waicAbund works for spNMix", {
  # as.vector gets rid of names
  waic.out <- as.vector(waicAbund(out, max(out$N.samples) + 10))
  expect_equal(length(waic.out), 3)
  expect_equal(waic.out[3], -2 * (waic.out[1] - waic.out[2]))
})

# Check fitted ------------------------
test_that("fitted works for spNMix", {
  fitted.out <- fitted(out)
  expect_equal(length(fitted.out), 2)
})

# Check predictions -------------------
test_that("predict works for spNMix", {
  X.0 <- cbind(X.0, X.re.0)
  colnames(X.0) <- c('(Intercept)', 'abund.cov.1', 'abund.cov.2', 'abund.factor.1', 'abund.factor.2')
  pred.out <- predict(out, X.0, coords.0, verbose = FALSE)
  expect_type(pred.out, "list")
  expect_equal(dim(pred.out$mu.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
  expect_equal(dim(pred.out$N.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
})
test_that("detection prediction works", {
  J.str <- 100
  X.p.0 <- matrix(1, nrow = J.str, ncol = p.det)
  pred.out <- predict(out, X.p.0, type = 'detection')
  expect_type(pred.out, 'list')
  expect_equal(dim(pred.out$p.0.samples), c(out$n.post * out$n.chains, J.str))
})

# Check PPCs --------------------------
test_that("posterior predictive checks work for spNMix", {
  n.post.samples <- out$n.post * out$n.chains
  ppc.out <- ppcAbund(out, 'chi-square', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))
  
  ppc.out <- ppcAbund(out, 'chi-square', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))

  ppc.out <- ppcAbund(out, 'chi-square', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))
})

# Abundance REs + covariates in all ---------------------------------------
set.seed(1000)
J.x <- 15
J.y <- 15
J <- J.x * J.y
n.rep <- sample(2:4, J, replace = TRUE)
beta <- c(0, -0.2, 0.2)
p.abund <- length(beta)
alpha <- c(0.5, -0.2, 0.3)
p.det <- length(alpha)
mu.RE <- list(levels = c(15, 10),
	       sigma.sq.mu = c(1.3, 0.3))
p.RE <- list()
phi <- runif(1, 3 / 1, 3 / .1)
sigma.sq <- 0.5
kappa <- 0.5
phi <- 3 / 0.5
nu <- 1
sp <- TRUE
cov.model <- 'matern'
family <- 'NB'
dat <- simNMix(J.x = J.x, J.y = J.y, n.rep = n.rep, beta = beta, alpha = alpha,
	       mu.RE = mu.RE, p.RE = p.RE, sp = TRUE, family = family, kappa = kappa, 
               sigma.sq = sigma.sq, phi = phi, nu = nu, cov.model = cov.model)

pred.indx <- sample(1:J, round(J * .25), replace = FALSE)
y <- dat$y[-pred.indx, , drop = FALSE]
y.0 <- dat$y[pred.indx, , drop = FALSE]
# Occupancy covariates
X <- dat$X[-pred.indx, , drop = FALSE]
X.re <- dat$X.re[-pred.indx, , drop = FALSE]
# Prediction covariates
X.0 <- dat$X[pred.indx, , drop = FALSE]
X.re.0 <- dat$X.re[pred.indx, , drop = FALSE]
# Detection covariates
X.p <- dat$X.p[-pred.indx, , , drop = FALSE]
# X.p.re <- dat$X.p.re[-pred.indx, , , drop = FALSE]
coords <- as.matrix(dat$coords[-pred.indx, ])
coords.0 <- as.matrix(dat$coords[pred.indx, ])

abund.covs <- cbind(X, X.re)
colnames(abund.covs) <- c('int', 'abund.cov.1', 'abund.cov.2', 'abund.factor.1', 'abund.factor.2')
det.covs <- list(det.cov.1 = X.p[, , 2], 
                 det.cov.2 = X.p[, , 3])
data.list <- list(y = y,
		  abund.covs = abund.covs,
 		  det.covs = det.covs, 
                  coords = coords)
# Priors
prior.list <- list(beta.normal = list(mean = 0, var = 10),
		   alpha.normal = list(mean = 0, var = 2.72),
                   sigma.sq.ig = c(2, 1),
                   phi.unif = c(3 / 1, 3 / .1), 
                   nu.unif = c(0.1, 5)) 

# Starting values
inits.list <- list(alpha = 0,
		   beta = 0,
		   kappa = 1.3,
		   sigma.sq.mu = 0.5,
		   N = apply(y, 1, max, na.rm = TRUE), 
                   phi = 3 / .5, sigma.sq = 0.5, nu = 2)
tuning <- list(beta = 0.1, alpha = 0.1, beta.star = 0.1, alpha.star = 0.1,
               kappa = 0.2, phi = 0.5, sigma.sq = 0.5, nu = 0.5, w = 1)
n.batch <- 10
batch.length <- 25
n.burn <- 50
n.thin <- 2
n.chains <- 2
abund.formula <- ~ (1 | abund.factor.1) + (1 | abund.factor.2) + abund.cov.1 + abund.cov.2
det.formula <- ~ det.cov.1 + det.cov.2

out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      inits = inits.list,
	      family = 'NB',
	      tuning = tuning,
	      priors = prior.list,
	      cov.model = 'matern', 
	      NNGP = TRUE, 
	      n.neighbors = 5,
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 1,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains,
              k.fold = 2,
              k.fold.threads = 1)

# Test to make sure it worked ---------
test_that("out is of class spNMix", {
  expect_s3_class(out, "spNMix")
})

# Check cross-validation --------------
test_that("cross-validation works", {
  expect_equal(length(out$rmspe), 1)
  expect_type(out$rmspe, "double")
  expect_gt(out$rmspe, 0)
})

# Check random effects ----------------
test_that("random effects are empty", {
  expect_equal(out$pRE, FALSE)
  expect_equal(out$muRE, TRUE)
})

# Check output data output is correct -
test_that("out$y == y", {
  expect_equal(out$y, y)
})

test_that("random effect gives error when non-numeric", {
  data.list$abund.covs <- as.data.frame(data.list$abund.covs)
  data.list$abund.covs$abund.factor.1 <- factor(data.list$abund.covs$abund.factor.1)
  expect_error(out <- spNMix(abund.formula = abund.formula,
	    det.formula = det.formula,
	    data = data.list,
	    n.batch = n.batch,
	    batch.length = batch.length,
	    inits = inits.list,
	    family = 'NB',
	    tuning = tuning,
	    priors = prior.list,
	    accept.rate = 0.43,
	    n.omp.threads = 1,
	    verbose = FALSE,
	    n.report = 50,
	    n.burn = n.burn,
	    n.thin = n.thin,
	    n.chains = n.chains))
  data.list$abund.covs$abund.factor.1 <- factor(data.list$abund.covs$abund.factor.1)
  expect_error(out <- spNMix(abund.formula = abund.formula,
	    det.formula = det.formula,
	    data = data.list,
	    n.batch = n.batch,
	    batch.length = batch.length,
	    inits = inits.list,
	    family = 'NB',
	    tuning = tuning,
	    priors = prior.list,
	    accept.rate = 0.43,
	    n.omp.threads = 1,
	    verbose = FALSE,
	    n.report = 50,
	    n.burn = n.burn,
	    n.thin = n.thin,
	    n.chains = n.chains))
})

# Check Poisson -----------------------
test_that("Poisson works", {
  out <- spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'spherical',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = FALSE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1)
  expect_s3_class(out, "spNMix")
})

# Check default priors ----------------
test_that("default priors, multiple chains, inits, burn, thin work", {
  out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      family = 'NB',
	      NNGP = TRUE,
	      n.neighbors = 5,
	      cov.model = 'exponential',
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 50,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains)
  expect_s3_class(out, 'spNMix')
})

# Check summary -----------------------
test_that("summary works", {
  expect_output(summary(out))
})

# Check verbose -----------------------
test_that("verbose prints to the screen", {
  expect_output(spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'gaussian',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = TRUE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1))
})

# Check waicAbund -----------------------
test_that("waicAbund works for spNMix", {
  # as.vector gets rid of names
  waic.out <- as.vector(waicAbund(out, max(out$N.samples) + 10))
  expect_equal(length(waic.out), 3)
  expect_equal(waic.out[3], -2 * (waic.out[1] - waic.out[2]))
})

# Check fitted ------------------------
test_that("fitted works for spNMix", {
  fitted.out <- fitted(out)
  expect_equal(length(fitted.out), 2)
})

# Check predictions -------------------
test_that("predict works for spNMix", {
  X.0 <- cbind(X.0, X.re.0)
  colnames(X.0) <- c('(Intercept)', 'abund.cov.1', 'abund.cov.2', 'abund.factor.1', 'abund.factor.2')
  pred.out <- predict(out, X.0, coords.0, verbose = FALSE)
  expect_type(pred.out, "list")
  expect_equal(dim(pred.out$mu.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
  expect_equal(dim(pred.out$N.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
})
test_that("detection prediction works", {
  X.p.0 <- dat$X.p[, 1, ]
  pred.out <- predict(out, X.p.0, type = 'detection')
  expect_type(pred.out, 'list')
  expect_equal(dim(pred.out$p.0.samples), c(out$n.post * out$n.chains, J))
})

# Check PPCs --------------------------
test_that("posterior predictive checks work for spNMix", {
  n.post.samples <- out$n.post * out$n.chains
  ppc.out <- ppcAbund(out, 'chi-square', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))
  
  ppc.out <- ppcAbund(out, 'chi-square', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))

  ppc.out <- ppcAbund(out, 'chi-square', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))
  
  ppc.out <- ppcAbund(out, 'freeman-tukey', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))
})

# Random intercepts on detection ------------------------------------------
set.seed(1000)
J.x <- 15
J.y <- 15
J <- J.x * J.y
n.rep <- sample(2:4, J, replace = TRUE)
beta <- c(0)
p.abund <- length(beta)
alpha <- c(0.5)
p.det <- length(alpha)
mu.RE <- list()
p.RE <- list(levels = c(25), 
             sigma.sq.p = c(0.5))
phi <- runif(1, 3 / 1, 3 / .1)
sigma.sq <- 0.5
kappa <- 0.5
phi <- 3 / 0.5
nu <- 1
sp <- TRUE
cov.model <- 'matern'
family <- 'NB'
dat <- simNMix(J.x = J.x, J.y = J.y, n.rep = n.rep, beta = beta, alpha = alpha,
	       mu.RE = mu.RE, p.RE = p.RE, sp = TRUE, family = family, kappa = kappa,
               sigma.sq = sigma.sq, phi = phi, nu = nu, cov.model = cov.model)

pred.indx <- sample(1:J, round(J * .25), replace = FALSE)
y <- dat$y[-pred.indx, , drop = FALSE]
y.0 <- dat$y[pred.indx, , drop = FALSE]
# Occupancy covariates
X <- dat$X[-pred.indx, , drop = FALSE]
# X.re <- dat$X.re[-pred.indx, , drop = FALSE]
# Prediction covariates
X.0 <- dat$X[pred.indx, , drop = FALSE]
# X.re.0 <- dat$X.re[pred.indx, , drop = FALSE]
# Detection covariates
X.p <- dat$X.p[-pred.indx, , , drop = FALSE]
X.p.re <- dat$X.p.re[-pred.indx, , , drop = FALSE]
coords <- as.matrix(dat$coords[-pred.indx, ])
coords.0 <- as.matrix(dat$coords[pred.indx, ])

abund.covs <- X
colnames(abund.covs) <- c('int')
det.covs <- list(int = X.p[, , 1], 
                 det.factor.1 = X.p.re[, , 1])
data.list <- list(y = y,
		  abund.covs = abund.covs,
 		  det.covs = det.covs,
                  coords = coords)
# Priors
prior.list <- list(beta.normal = list(mean = 0, var = 10),
		   alpha.normal = list(mean = 0, var = 2.72),
                   sigma.sq.ig = c(2, 1),
                   phi.unif = c(3 / 1, 3 / .1),
                   nu.unif = c(0.1, 5))

# Starting values
inits.list <- list(alpha = 0,
		   beta = 0,
		   kappa = 1.3,
		   sigma.sq.mu = 0.5,
		   N = apply(y, 1, max, na.rm = TRUE),
                   phi = 3 / .5, sigma.sq = 0.5, nu = 2)
tuning <- list(beta = 0.1, alpha = 0.1, beta.star = 0.1, alpha.star = 0.1,
               kappa = 0.2, phi = 0.5, sigma.sq = 0.5, nu = 0.5, w = 1)
n.batch <- 10
batch.length <- 25
n.burn <- 50
n.thin <- 2
n.chains <- 2
abund.formula <- ~ 1
det.formula <- ~ (1 | det.factor.1)

out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      inits = inits.list,
	      family = 'NB',
	      tuning = tuning,
	      priors = prior.list,
	      cov.model = 'matern',
	      NNGP = TRUE,
	      n.neighbors = 5,
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 1,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains,
              k.fold = 2,
              k.fold.threads = 1)

# Test to make sure it worked ---------
test_that("out is of class spNMix", {
  expect_s3_class(out, "spNMix")
})

# Check cross-validation --------------
test_that("cross-validation works", {
  expect_equal(length(out$rmspe), 1)
  expect_type(out$rmspe, "double")
  expect_gt(out$rmspe, 0)
})

# Check random effects ----------------
test_that("random effects are empty", {
  expect_equal(out$pRE, TRUE)
  expect_equal(out$muRE, FALSE)
})

test_that("random effect gives error when non-numeric", {
  data.list$det.covs$det.factor.1 <- factor(data.list$det.covs$det.factor.1)
  expect_error(out <- spNMix(abund.formula = abund.formula,
	    det.formula = det.formula,
	    data = data.list,
	    n.batch = n.batch,
	    batch.length = batch.length,
	    inits = inits.list,
	    family = 'NB',
	    tuning = tuning,
	    priors = prior.list,
	    accept.rate = 0.43,
	    n.omp.threads = 1,
	    verbose = FALSE,
	    n.report = 50,
	    n.burn = n.burn,
	    n.thin = n.thin,
	    n.chains = n.chains))
  data.list$det.covs$det.factor.1 <- as.character(factor(data.list$det.covs$det.factor.1))
  expect_error(out <- spNMix(abund.formula = abund.formula,
	    det.formula = det.formula,
	    data = data.list,
	    n.batch = n.batch,
	    batch.length = batch.length,
	    inits = inits.list,
	    family = 'NB',
	    tuning = tuning,
	    priors = prior.list,
	    accept.rate = 0.43,
	    n.omp.threads = 1,
	    verbose = FALSE,
	    n.report = 50,
	    n.burn = n.burn,
	    n.thin = n.thin,
	    n.chains = n.chains))
})


# Check output data output is correct -
test_that("out$y == y", {
  expect_equal(out$y, y)
})

# Check Poisson -----------------------
test_that("Poisson works", {
  out <- spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'spherical',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = FALSE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1)
  expect_s3_class(out, "spNMix")
})

# Check default priors ----------------
test_that("default priors, multiple chains, inits, burn, thin work", {
  out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      family = 'NB',
	      NNGP = TRUE,
	      n.neighbors = 5,
	      cov.model = 'exponential',
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 50,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains)
  expect_s3_class(out, 'spNMix')
})

# Check summary -----------------------
test_that("summary works", {
  expect_output(summary(out))
})

# Check verbose -----------------------
test_that("verbose prints to the screen", {
  expect_output(spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'gaussian',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = TRUE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1))
})

# Check waicAbund -----------------------
test_that("waicAbund works for spNMix", {
  # as.vector gets rid of names
  waic.out <- as.vector(waicAbund(out, max(out$N.samples) + 10))
  expect_equal(length(waic.out), 3)
  expect_equal(waic.out[3], -2 * (waic.out[1] - waic.out[2]))
})

# Check fitted ------------------------
test_that("fitted works for spNMix", {
  fitted.out <- fitted(out)
  expect_equal(length(fitted.out), 2)
})

# Check predictions -------------------
test_that("predict works for spNMix", {
  pred.out <- predict(out, X.0, coords.0, verbose = FALSE)
  expect_type(pred.out, "list")
  expect_equal(dim(pred.out$mu.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
  expect_equal(dim(pred.out$N.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
})
test_that("detection prediction works", {
  X.p.0 <- cbind(dat$X.p[, 1, ], dat$X.p.re[, 1, ])
  colnames(X.p.0) <- c('(Intercept)', 'det.factor.1')
  pred.out <- predict(out, X.p.0, type = 'detection')
  expect_type(pred.out, 'list')
  expect_equal(dim(pred.out$p.0.samples), c(out$n.post * out$n.chains, J))
})

# Check PPCs --------------------------
test_that("posterior predictive checks work for spNMix", {
  n.post.samples <- out$n.post * out$n.chains
  ppc.out <- ppcAbund(out, 'chi-square', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))

  ppc.out <- ppcAbund(out, 'chi-square', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))

  ppc.out <- ppcAbund(out, 'freeman-tukey', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))

  ppc.out <- ppcAbund(out, 'freeman-tukey', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))

  ppc.out <- ppcAbund(out, 'chi-square', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))

  ppc.out <- ppcAbund(out, 'freeman-tukey', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))
})

# Multiple random intercepts on detection ---------------------------------
set.seed(1000)
J.x <- 15
J.y <- 15
J <- J.x * J.y
n.rep <- sample(2:4, J, replace = TRUE)
beta <- c(0)
p.abund <- length(beta)
alpha <- c(0.5)
p.det <- length(alpha)
mu.RE <- list()
p.RE <- list(levels = c(25, 15), 
             sigma.sq.p = c(0.5, 0.7))
phi <- runif(1, 3 / 1, 3 / .1)
sigma.sq <- 0.5
kappa <- 0.5
phi <- 3 / 0.5
nu <- 1
sp <- TRUE
cov.model <- 'matern'
family <- 'NB'
dat <- simNMix(J.x = J.x, J.y = J.y, n.rep = n.rep, beta = beta, alpha = alpha,
	       mu.RE = mu.RE, p.RE = p.RE, sp = TRUE, family = family, kappa = kappa,
               sigma.sq = sigma.sq, phi = phi, nu = nu, cov.model = cov.model)

pred.indx <- sample(1:J, round(J * .25), replace = FALSE)
y <- dat$y[-pred.indx, , drop = FALSE]
y.0 <- dat$y[pred.indx, , drop = FALSE]
# Occupancy covariates
X <- dat$X[-pred.indx, , drop = FALSE]
# X.re <- dat$X.re[-pred.indx, , drop = FALSE]
# Prediction covariates
X.0 <- dat$X[pred.indx, , drop = FALSE]
# X.re.0 <- dat$X.re[pred.indx, , drop = FALSE]
# Detection covariates
X.p <- dat$X.p[-pred.indx, , , drop = FALSE]
X.p.re <- dat$X.p.re[-pred.indx, , , drop = FALSE]
coords <- as.matrix(dat$coords[-pred.indx, ])
coords.0 <- as.matrix(dat$coords[pred.indx, ])

abund.covs <- X
colnames(abund.covs) <- c('int')
det.covs <- list(int = X.p[, , 1], 
                 det.factor.1 = X.p.re[, , 1], 
                 det.factor.2 = X.p.re[, , 2])
data.list <- list(y = y,
		  abund.covs = abund.covs,
 		  det.covs = det.covs,
                  coords = coords)
# Priors
prior.list <- list(beta.normal = list(mean = 0, var = 10),
		   alpha.normal = list(mean = 0, var = 2.72),
                   sigma.sq.ig = c(2, 1),
                   phi.unif = c(3 / 1, 3 / .1),
                   nu.unif = c(0.1, 5))

# Starting values
inits.list <- list(alpha = 0,
		   beta = 0,
		   kappa = 1.3,
		   sigma.sq.mu = 0.5,
		   N = apply(y, 1, max, na.rm = TRUE),
                   phi = 3 / .5, sigma.sq = 0.5, nu = 2)
tuning <- list(beta = 0.1, alpha = 0.1, beta.star = 0.1, alpha.star = 0.1,
               kappa = 0.2, phi = 0.5, sigma.sq = 0.5, nu = 0.5, w = 1)
n.batch <- 10
batch.length <- 25
n.burn <- 50
n.thin <- 2
n.chains <- 2
abund.formula <- ~ 1
det.formula <- ~ (1 | det.factor.1) + (1 | det.factor.2)

out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      inits = inits.list,
	      family = 'NB',
	      tuning = tuning,
	      priors = prior.list,
	      cov.model = 'matern',
	      NNGP = TRUE,
	      n.neighbors = 5,
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 1,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains,
              k.fold = 2,
              k.fold.threads = 1)

# Test to make sure it worked ---------
test_that("out is of class spNMix", {
  expect_s3_class(out, "spNMix")
})

# Check cross-validation --------------
test_that("cross-validation works", {
  expect_equal(length(out$rmspe), 1)
  expect_type(out$rmspe, "double")
  expect_gt(out$rmspe, 0)
})

# Check random effects ----------------
test_that("random effects are empty", {
  expect_equal(out$pRE, TRUE)
  expect_equal(out$muRE, FALSE)
})

test_that("random effect gives error when non-numeric", {
  data.list$det.covs$det.factor.1 <- factor(data.list$det.covs$det.factor.1)
  expect_error(out <- spNMix(abund.formula = abund.formula,
	    det.formula = det.formula,
	    data = data.list,
	    n.batch = n.batch,
	    batch.length = batch.length,
	    inits = inits.list,
	    family = 'NB',
	    tuning = tuning,
	    priors = prior.list,
	    accept.rate = 0.43,
	    n.omp.threads = 1,
	    verbose = FALSE,
	    n.report = 50,
	    n.burn = n.burn,
	    n.thin = n.thin,
	    n.chains = n.chains))
  data.list$det.covs$det.factor.1 <- as.character(factor(data.list$det.covs$det.factor.1))
  expect_error(out <- spNMix(abund.formula = abund.formula,
	    det.formula = det.formula,
	    data = data.list,
	    n.batch = n.batch,
	    batch.length = batch.length,
	    inits = inits.list,
	    family = 'NB',
	    tuning = tuning,
	    priors = prior.list,
	    accept.rate = 0.43,
	    n.omp.threads = 1,
	    verbose = FALSE,
	    n.report = 50,
	    n.burn = n.burn,
	    n.thin = n.thin,
	    n.chains = n.chains))
})


# Check output data output is correct -
test_that("out$y == y", {
  expect_equal(out$y, y)
})

# Check Poisson -----------------------
test_that("Poisson works", {
  out <- spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'spherical',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = FALSE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1)
  expect_s3_class(out, "spNMix")
})

# Check default priors ----------------
test_that("default priors, multiple chains, inits, burn, thin work", {
  out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      family = 'NB',
	      NNGP = TRUE,
	      n.neighbors = 5,
	      cov.model = 'exponential',
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 50,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains)
  expect_s3_class(out, 'spNMix')
})

# Check summary -----------------------
test_that("summary works", {
  expect_output(summary(out))
})

# Check verbose -----------------------
test_that("verbose prints to the screen", {
  expect_output(spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'gaussian',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = TRUE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1))
})

# Check waicAbund -----------------------
test_that("waicAbund works for spNMix", {
  # as.vector gets rid of names
  waic.out <- as.vector(waicAbund(out, max(out$N.samples) + 10))
  expect_equal(length(waic.out), 3)
  expect_equal(waic.out[3], -2 * (waic.out[1] - waic.out[2]))
})

# Check fitted ------------------------
test_that("fitted works for spNMix", {
  fitted.out <- fitted(out)
  expect_equal(length(fitted.out), 2)
})

# Check predictions -------------------
test_that("predict works for spNMix", {
  pred.out <- predict(out, X.0, coords.0, verbose = FALSE)
  expect_type(pred.out, "list")
  expect_equal(dim(pred.out$mu.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
  expect_equal(dim(pred.out$N.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
})
test_that("detection prediction works", {
  X.p.0 <- cbind(dat$X.p[, 1, ], dat$X.p.re[, 1, ])
  colnames(X.p.0) <- c('(Intercept)', 'det.factor.1', 'det.factor.2')
  pred.out <- predict(out, X.p.0, type = 'detection')
  expect_type(pred.out, 'list')
  expect_equal(dim(pred.out$p.0.samples), c(out$n.post * out$n.chains, J))
})

# Check PPCs --------------------------
test_that("posterior predictive checks work for spNMix", {
  n.post.samples <- out$n.post * out$n.chains
  ppc.out <- ppcAbund(out, 'chi-square', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))

  ppc.out <- ppcAbund(out, 'chi-square', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))

  ppc.out <- ppcAbund(out, 'freeman-tukey', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))

  ppc.out <- ppcAbund(out, 'freeman-tukey', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))

  ppc.out <- ppcAbund(out, 'chi-square', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))

  ppc.out <- ppcAbund(out, 'freeman-tukey', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))
})

# Multiple random intercepts with covariates ------------------------------
set.seed(1000)
J.x <- 15
J.y <- 15
J <- J.x * J.y
n.rep <- sample(2:4, J, replace = TRUE)
beta <- c(0)
p.abund <- length(beta)
alpha <- c(0.5, -0.2)
p.det <- length(alpha)
mu.RE <- list()
p.RE <- list(levels = c(25, 15), 
             sigma.sq.p = c(0.5, 0.7))
phi <- runif(1, 3 / 1, 3 / .1)
sigma.sq <- 0.5
kappa <- 0.5
phi <- 3 / 0.5
nu <- 1
sp <- TRUE
cov.model <- 'matern'
family <- 'NB'
dat <- simNMix(J.x = J.x, J.y = J.y, n.rep = n.rep, beta = beta, alpha = alpha,
	       mu.RE = mu.RE, p.RE = p.RE, sp = TRUE, family = family, kappa = kappa,
               sigma.sq = sigma.sq, phi = phi, nu = nu, cov.model = cov.model)

pred.indx <- sample(1:J, round(J * .25), replace = FALSE)
y <- dat$y[-pred.indx, , drop = FALSE]
y.0 <- dat$y[pred.indx, , drop = FALSE]
# Occupancy covariates
X <- dat$X[-pred.indx, , drop = FALSE]
# X.re <- dat$X.re[-pred.indx, , drop = FALSE]
# Prediction covariates
X.0 <- dat$X[pred.indx, , drop = FALSE]
# X.re.0 <- dat$X.re[pred.indx, , drop = FALSE]
# Detection covariates
X.p <- dat$X.p[-pred.indx, , , drop = FALSE]
X.p.re <- dat$X.p.re[-pred.indx, , , drop = FALSE]
coords <- as.matrix(dat$coords[-pred.indx, ])
coords.0 <- as.matrix(dat$coords[pred.indx, ])

abund.covs <- X
colnames(abund.covs) <- c('int')
det.covs <- list(det.cov.1 = X.p[, , 2], 
                 det.factor.1 = X.p.re[, , 1], 
                 det.factor.2 = X.p.re[, , 2])
data.list <- list(y = y,
		  abund.covs = abund.covs,
 		  det.covs = det.covs,
                  coords = coords)
# Priors
prior.list <- list(beta.normal = list(mean = 0, var = 10),
		   alpha.normal = list(mean = 0, var = 2.72),
                   sigma.sq.ig = c(2, 1),
                   phi.unif = c(3 / 1, 3 / .1),
                   nu.unif = c(0.1, 5))

# Starting values
inits.list <- list(alpha = 0,
		   beta = 0,
		   kappa = 1.3,
		   sigma.sq.mu = 0.5,
		   N = apply(y, 1, max, na.rm = TRUE),
                   phi = 3 / .5, sigma.sq = 0.5, nu = 2)
tuning <- list(beta = 0.1, alpha = 0.1, beta.star = 0.1, alpha.star = 0.1,
               kappa = 0.2, phi = 0.5, sigma.sq = 0.5, nu = 0.5, w = 1)
n.batch <- 10
batch.length <- 25
n.burn <- 50
n.thin <- 2
n.chains <- 2
abund.formula <- ~ 1
det.formula <- ~ det.cov.1 + (1 | det.factor.1) + (1 | det.factor.2)

out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      inits = inits.list,
	      family = 'NB',
	      tuning = tuning,
	      priors = prior.list,
	      cov.model = 'matern',
	      NNGP = TRUE,
	      n.neighbors = 5,
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 1,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains,
              k.fold = 2,
              k.fold.threads = 1)

# Test to make sure it worked ---------
test_that("out is of class spNMix", {
  expect_s3_class(out, "spNMix")
})

# Check cross-validation --------------
test_that("cross-validation works", {
  expect_equal(length(out$rmspe), 1)
  expect_type(out$rmspe, "double")
  expect_gt(out$rmspe, 0)
})

# Check random effects ----------------
test_that("random effects are empty", {
  expect_equal(out$pRE, TRUE)
  expect_equal(out$muRE, FALSE)
})

test_that("random effect gives error when non-numeric", {
  data.list$det.covs$det.factor.1 <- factor(data.list$det.covs$det.factor.1)
  expect_error(out <- spNMix(abund.formula = abund.formula,
	    det.formula = det.formula,
	    data = data.list,
	    n.batch = n.batch,
	    batch.length = batch.length,
	    inits = inits.list,
	    family = 'NB',
	    tuning = tuning,
	    priors = prior.list,
	    accept.rate = 0.43,
	    n.omp.threads = 1,
	    verbose = FALSE,
	    n.report = 50,
	    n.burn = n.burn,
	    n.thin = n.thin,
	    n.chains = n.chains))
  data.list$det.covs$det.factor.1 <- as.character(factor(data.list$det.covs$det.factor.1))
  expect_error(out <- spNMix(abund.formula = abund.formula,
	    det.formula = det.formula,
	    data = data.list,
	    n.batch = n.batch,
	    batch.length = batch.length,
	    inits = inits.list,
	    family = 'NB',
	    tuning = tuning,
	    priors = prior.list,
	    accept.rate = 0.43,
	    n.omp.threads = 1,
	    verbose = FALSE,
	    n.report = 50,
	    n.burn = n.burn,
	    n.thin = n.thin,
	    n.chains = n.chains))
})


# Check output data output is correct -
test_that("out$y == y", {
  expect_equal(out$y, y)
})

# Check Poisson -----------------------
test_that("Poisson works", {
  out <- spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'spherical',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = FALSE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1)
  expect_s3_class(out, "spNMix")
})

# Check default priors ----------------
test_that("default priors, multiple chains, inits, burn, thin work", {
  out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      family = 'NB',
	      NNGP = TRUE,
	      n.neighbors = 5,
	      cov.model = 'exponential',
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 50,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains)
  expect_s3_class(out, 'spNMix')
})

# Check summary -----------------------
test_that("summary works", {
  expect_output(summary(out))
})

# Check verbose -----------------------
test_that("verbose prints to the screen", {
  expect_output(spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'gaussian',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = TRUE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1))
})

# Check waicAbund -----------------------
test_that("waicAbund works for spNMix", {
  # as.vector gets rid of names
  waic.out <- as.vector(waicAbund(out, max(out$N.samples) + 10))
  expect_equal(length(waic.out), 3)
  expect_equal(waic.out[3], -2 * (waic.out[1] - waic.out[2]))
})

# Check fitted ------------------------
test_that("fitted works for spNMix", {
  fitted.out <- fitted(out)
  expect_equal(length(fitted.out), 2)
})

# Check predictions -------------------
test_that("predict works for spNMix", {
  pred.out <- predict(out, X.0, coords.0, verbose = FALSE)
  expect_type(pred.out, "list")
  expect_equal(dim(pred.out$mu.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
  expect_equal(dim(pred.out$N.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
})
test_that("detection prediction works", {
  X.p.0 <- cbind(dat$X.p[, 1, ], dat$X.p.re[, 1, ])
  colnames(X.p.0) <- c('(Intercept)', 'det.cov.1', 'det.factor.1', 'det.factor.2')
  pred.out <- predict(out, X.p.0, type = 'detection')
  expect_type(pred.out, 'list')
  expect_equal(dim(pred.out$p.0.samples), c(out$n.post * out$n.chains, J))
})

# Check PPCs --------------------------
test_that("posterior predictive checks work for spNMix", {
  n.post.samples <- out$n.post * out$n.chains
  ppc.out <- ppcAbund(out, 'chi-square', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))

  ppc.out <- ppcAbund(out, 'chi-square', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))

  ppc.out <- ppcAbund(out, 'freeman-tukey', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))

  ppc.out <- ppcAbund(out, 'freeman-tukey', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))

  ppc.out <- ppcAbund(out, 'chi-square', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))

  ppc.out <- ppcAbund(out, 'freeman-tukey', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))
})

# Multiple random intercepts with covariates on both ----------------------
set.seed(1000)
J.x <- 15
J.y <- 15
J <- J.x * J.y
n.rep <- sample(2:4, J, replace = TRUE)
beta <- c(0, -0.3)
p.abund <- length(beta)
alpha <- c(0.5, -0.2)
p.det <- length(alpha)
mu.RE <- list()
p.RE <- list(levels = c(25, 15), 
             sigma.sq.p = c(0.5, 0.7))
phi <- runif(1, 3 / 1, 3 / .1)
sigma.sq <- 0.5
kappa <- 0.5
phi <- 3 / 0.5
nu <- 1
sp <- TRUE
cov.model <- 'matern'
family <- 'NB'
dat <- simNMix(J.x = J.x, J.y = J.y, n.rep = n.rep, beta = beta, alpha = alpha,
	       mu.RE = mu.RE, p.RE = p.RE, sp = TRUE, family = family, kappa = kappa,
               sigma.sq = sigma.sq, phi = phi, nu = nu, cov.model = cov.model)

pred.indx <- sample(1:J, round(J * .25), replace = FALSE)
y <- dat$y[-pred.indx, , drop = FALSE]
y.0 <- dat$y[pred.indx, , drop = FALSE]
# Occupancy covariates
X <- dat$X[-pred.indx, , drop = FALSE]
# X.re <- dat$X.re[-pred.indx, , drop = FALSE]
# Prediction covariates
X.0 <- dat$X[pred.indx, , drop = FALSE]
# X.re.0 <- dat$X.re[pred.indx, , drop = FALSE]
# Detection covariates
X.p <- dat$X.p[-pred.indx, , , drop = FALSE]
X.p.re <- dat$X.p.re[-pred.indx, , , drop = FALSE]
coords <- as.matrix(dat$coords[-pred.indx, ])
coords.0 <- as.matrix(dat$coords[pred.indx, ])

abund.covs <- X
colnames(abund.covs) <- c('int', 'abund.cov.1')
det.covs <- list(det.cov.1 = X.p[, , 2], 
                 det.factor.1 = X.p.re[, , 1], 
                 det.factor.2 = X.p.re[, , 2])
data.list <- list(y = y,
		  abund.covs = abund.covs,
 		  det.covs = det.covs,
                  coords = coords)
# Priors
prior.list <- list(beta.normal = list(mean = 0, var = 10),
		   alpha.normal = list(mean = 0, var = 2.72),
                   sigma.sq.ig = c(2, 1),
                   phi.unif = c(3 / 1, 3 / .1),
                   nu.unif = c(0.1, 5))

# Starting values
inits.list <- list(alpha = 0,
		   beta = 0,
		   kappa = 1.3,
		   sigma.sq.mu = 0.5,
		   N = apply(y, 1, max, na.rm = TRUE),
                   phi = 3 / .5, sigma.sq = 0.5, nu = 2)
tuning <- list(beta = 0.1, alpha = 0.1, beta.star = 0.1, alpha.star = 0.1,
               kappa = 0.2, phi = 0.5, sigma.sq = 0.5, nu = 0.5, w = 1)
n.batch <- 10
batch.length <- 25
n.burn <- 50
n.thin <- 2
n.chains <- 2
abund.formula <- ~ abund.cov.1
det.formula <- ~ det.cov.1 + (1 | det.factor.1) + (1 | det.factor.2)

out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      inits = inits.list,
	      family = 'NB',
	      tuning = tuning,
	      priors = prior.list,
	      cov.model = 'matern',
	      NNGP = TRUE,
	      n.neighbors = 5,
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 1,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains,
              k.fold = 2,
              k.fold.threads = 1)

# Test to make sure it worked ---------
test_that("out is of class spNMix", {
  expect_s3_class(out, "spNMix")
})

# Check cross-validation --------------
test_that("cross-validation works", {
  expect_equal(length(out$rmspe), 1)
  expect_type(out$rmspe, "double")
  expect_gt(out$rmspe, 0)
})

# Check random effects ----------------
test_that("random effects are empty", {
  expect_equal(out$pRE, TRUE)
  expect_equal(out$muRE, FALSE)
})

test_that("random effect gives error when non-numeric", {
  data.list$det.covs$det.factor.1 <- factor(data.list$det.covs$det.factor.1)
  expect_error(out <- spNMix(abund.formula = abund.formula,
	    det.formula = det.formula,
	    data = data.list,
	    n.batch = n.batch,
	    batch.length = batch.length,
	    inits = inits.list,
	    family = 'NB',
	    tuning = tuning,
	    priors = prior.list,
	    accept.rate = 0.43,
	    n.omp.threads = 1,
	    verbose = FALSE,
	    n.report = 50,
	    n.burn = n.burn,
	    n.thin = n.thin,
	    n.chains = n.chains))
  data.list$det.covs$det.factor.1 <- as.character(factor(data.list$det.covs$det.factor.1))
  expect_error(out <- spNMix(abund.formula = abund.formula,
	    det.formula = det.formula,
	    data = data.list,
	    n.batch = n.batch,
	    batch.length = batch.length,
	    inits = inits.list,
	    family = 'NB',
	    tuning = tuning,
	    priors = prior.list,
	    accept.rate = 0.43,
	    n.omp.threads = 1,
	    verbose = FALSE,
	    n.report = 50,
	    n.burn = n.burn,
	    n.thin = n.thin,
	    n.chains = n.chains))
})


# Check output data output is correct -
test_that("out$y == y", {
  expect_equal(out$y, y)
})

# Check Poisson -----------------------
test_that("Poisson works", {
  out <- spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'spherical',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = FALSE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1)
  expect_s3_class(out, "spNMix")
})

# Check default priors ----------------
test_that("default priors, multiple chains, inits, burn, thin work", {
  out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      family = 'NB',
	      NNGP = TRUE,
	      n.neighbors = 5,
	      cov.model = 'exponential',
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 50,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains)
  expect_s3_class(out, 'spNMix')
})

# Check summary -----------------------
test_that("summary works", {
  expect_output(summary(out))
})

# Check verbose -----------------------
test_that("verbose prints to the screen", {
  expect_output(spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'gaussian',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = TRUE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1))
})

# Check waicAbund -----------------------
test_that("waicAbund works for spNMix", {
  # as.vector gets rid of names
  waic.out <- as.vector(waicAbund(out, max(out$N.samples) + 10))
  expect_equal(length(waic.out), 3)
  expect_equal(waic.out[3], -2 * (waic.out[1] - waic.out[2]))
})

# Check fitted ------------------------
test_that("fitted works for spNMix", {
  fitted.out <- fitted(out)
  expect_equal(length(fitted.out), 2)
})

# Check predictions -------------------
test_that("predict works for spNMix", {
  pred.out <- predict(out, X.0, coords.0, verbose = FALSE)
  expect_type(pred.out, "list")
  expect_equal(dim(pred.out$mu.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
  expect_equal(dim(pred.out$N.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
})
test_that("detection prediction works", {
  X.p.0 <- cbind(dat$X.p[, 1, ], dat$X.p.re[, 1, ])
  colnames(X.p.0) <- c('(Intercept)', 'det.cov.1', 'det.factor.1', 'det.factor.2')
  pred.out <- predict(out, X.p.0, type = 'detection')
  expect_type(pred.out, 'list')
  expect_equal(dim(pred.out$p.0.samples), c(out$n.post * out$n.chains, J))
})

# Check PPCs --------------------------
test_that("posterior predictive checks work for spNMix", {
  n.post.samples <- out$n.post * out$n.chains
  ppc.out <- ppcAbund(out, 'chi-square', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))

  ppc.out <- ppcAbund(out, 'chi-square', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))

  ppc.out <- ppcAbund(out, 'freeman-tukey', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))

  ppc.out <- ppcAbund(out, 'freeman-tukey', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))

  ppc.out <- ppcAbund(out, 'chi-square', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))

  ppc.out <- ppcAbund(out, 'freeman-tukey', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))
})

# Multiple random intercepts and covariates -------------------------------
set.seed(1000)
J.x <- 15
J.y <- 15
J <- J.x * J.y
n.rep <- sample(2:4, J, replace = TRUE)
beta <- c(0, -0.3)
p.abund <- length(beta)
alpha <- c(0.5, -0.2)
p.det <- length(alpha)
mu.RE <- list(levels = c(10, 15), 
              sigma.sq.mu = c(0.3, 0.9))
p.RE <- list(levels = c(25, 15), 
             sigma.sq.p = c(0.5, 0.7))
phi <- runif(1, 3 / 1, 3 / .1)
sigma.sq <- 0.5
kappa <- 0.5
phi <- 3 / 0.5
nu <- 1
sp <- TRUE
cov.model <- 'matern'
family <- 'NB'
dat <- simNMix(J.x = J.x, J.y = J.y, n.rep = n.rep, beta = beta, alpha = alpha,
	       mu.RE = mu.RE, p.RE = p.RE, sp = TRUE, family = family, kappa = kappa,
               sigma.sq = sigma.sq, phi = phi, nu = nu, cov.model = cov.model)

pred.indx <- sample(1:J, round(J * .25), replace = FALSE)
y <- dat$y[-pred.indx, , drop = FALSE]
y.0 <- dat$y[pred.indx, , drop = FALSE]
# Occupancy covariates
X <- dat$X[-pred.indx, , drop = FALSE]
X.re <- dat$X.re[-pred.indx, , drop = FALSE]
# Prediction covariates
X.0 <- dat$X[pred.indx, , drop = FALSE]
X.re.0 <- dat$X.re[pred.indx, , drop = FALSE]
# Detection covariates
X.p <- dat$X.p[-pred.indx, , , drop = FALSE]
X.p.re <- dat$X.p.re[-pred.indx, , , drop = FALSE]
coords <- as.matrix(dat$coords[-pred.indx, ])
coords.0 <- as.matrix(dat$coords[pred.indx, ])

abund.covs <- cbind(X, X.re)
colnames(abund.covs) <- c('int', 'abund.cov.1', 'abund.factor.1', 'abund.factor.2')
det.covs <- list(det.cov.1 = X.p[, , 2], 
                 det.factor.1 = X.p.re[, , 1], 
                 det.factor.2 = X.p.re[, , 2])
data.list <- list(y = y,
		  abund.covs = abund.covs,
 		  det.covs = det.covs,
                  coords = coords)
# Priors
prior.list <- list(beta.normal = list(mean = 0, var = 10),
		   alpha.normal = list(mean = 0, var = 2.72),
                   sigma.sq.ig = c(2, 1),
                   phi.unif = c(3 / 1, 3 / .1),
                   nu.unif = c(0.1, 5))

# Starting values
inits.list <- list(alpha = 0,
		   beta = 0,
		   kappa = 1.3,
		   sigma.sq.mu = 0.5,
		   N = apply(y, 1, max, na.rm = TRUE),
                   phi = 3 / .5, sigma.sq = 0.5, nu = 2)
tuning <- list(beta = 0.1, alpha = 0.1, beta.star = 0.1, alpha.star = 0.1,
               kappa = 0.2, phi = 0.5, sigma.sq = 0.5, nu = 0.5, w = 1)
n.batch <- 10
batch.length <- 25
n.burn <- 50
n.thin <- 2
n.chains <- 2
abund.formula <- ~ abund.cov.1 + (1 | abund.factor.1) + (1 | abund.factor.2)
det.formula <- ~ det.cov.1 + (1 | det.factor.1) + (1 | det.factor.2)

out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      inits = inits.list,
	      family = 'NB',
	      tuning = tuning,
	      priors = prior.list,
	      cov.model = 'matern',
	      NNGP = TRUE,
	      n.neighbors = 5,
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 1,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains,
              k.fold = 2,
              k.fold.threads = 1)

# Test to make sure it worked ---------
test_that("out is of class spNMix", {
  expect_s3_class(out, "spNMix")
})

# Check cross-validation --------------
test_that("cross-validation works", {
  expect_equal(length(out$rmspe), 1)
  expect_type(out$rmspe, "double")
  expect_gt(out$rmspe, 0)
})

# Check random effects ----------------
test_that("random effects are empty", {
  expect_equal(out$pRE, TRUE)
  expect_equal(out$muRE, TRUE)
})

test_that("random effect gives error when non-numeric", {
  data.list$det.covs$det.factor.1 <- factor(data.list$det.covs$det.factor.1)
  expect_error(out <- spNMix(abund.formula = abund.formula,
	    det.formula = det.formula,
	    data = data.list,
	    n.batch = n.batch,
	    batch.length = batch.length,
	    inits = inits.list,
	    family = 'NB',
	    tuning = tuning,
	    priors = prior.list,
	    accept.rate = 0.43,
	    n.omp.threads = 1,
	    verbose = FALSE,
	    n.report = 50,
	    n.burn = n.burn,
	    n.thin = n.thin,
	    n.chains = n.chains))
  data.list$det.covs$det.factor.1 <- as.character(factor(data.list$det.covs$det.factor.1))
  expect_error(out <- spNMix(abund.formula = abund.formula,
	    det.formula = det.formula,
	    data = data.list,
	    n.batch = n.batch,
	    batch.length = batch.length,
	    inits = inits.list,
	    family = 'NB',
	    tuning = tuning,
	    priors = prior.list,
	    accept.rate = 0.43,
	    n.omp.threads = 1,
	    verbose = FALSE,
	    n.report = 50,
	    n.burn = n.burn,
	    n.thin = n.thin,
	    n.chains = n.chains))
})


# Check output data output is correct -
test_that("out$y == y", {
  expect_equal(out$y, y)
})

# Check Poisson -----------------------
test_that("Poisson works", {
  out <- spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'spherical',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = FALSE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1)
  expect_s3_class(out, "spNMix")
})

# Check default priors ----------------
test_that("default priors, multiple chains, inits, burn, thin work", {
  out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      family = 'NB',
	      NNGP = TRUE,
	      n.neighbors = 5,
	      cov.model = 'exponential',
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 50,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains)
  expect_s3_class(out, 'spNMix')
})

# Check summary -----------------------
test_that("summary works", {
  expect_output(summary(out))
})

# Check verbose -----------------------
test_that("verbose prints to the screen", {
  expect_output(spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'gaussian',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = TRUE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1))
})

# Check waicAbund -----------------------
test_that("waicAbund works for spNMix", {
  # as.vector gets rid of names
  waic.out <- as.vector(waicAbund(out, max(out$N.samples) + 10))
  expect_equal(length(waic.out), 3)
  expect_equal(waic.out[3], -2 * (waic.out[1] - waic.out[2]))
})

# Check fitted ------------------------
test_that("fitted works for spNMix", {
  fitted.out <- fitted(out)
  expect_equal(length(fitted.out), 2)
})

# Check predictions -------------------
test_that("predict works for spNMix", {
  X.0 <- cbind(X.0, X.re.0)
  colnames(X.0) <- c('(Intercept)', 'abund.cov.1', 'abund.factor.1', 'abund.factor.2')
  pred.out <- predict(out, X.0, coords.0, verbose = FALSE)
  expect_type(pred.out, "list")
  expect_equal(dim(pred.out$mu.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
  expect_equal(dim(pred.out$N.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
})
test_that("detection prediction works", {
  X.p.0 <- cbind(dat$X.p[, 1, ], dat$X.p.re[, 1, ])
  colnames(X.p.0) <- c('(Intercept)', 'det.cov.1', 'det.factor.1', 'det.factor.2')
  pred.out <- predict(out, X.p.0, type = 'detection')
  expect_type(pred.out, 'list')
  expect_equal(dim(pred.out$p.0.samples), c(out$n.post * out$n.chains, J))
})

# Check PPCs --------------------------
test_that("posterior predictive checks work for spNMix", {
  n.post.samples <- out$n.post * out$n.chains
  ppc.out <- ppcAbund(out, 'chi-square', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))

  ppc.out <- ppcAbund(out, 'chi-square', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))

  ppc.out <- ppcAbund(out, 'freeman-tukey', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))

  ppc.out <- ppcAbund(out, 'freeman-tukey', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))

  ppc.out <- ppcAbund(out, 'chi-square', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))

  ppc.out <- ppcAbund(out, 'freeman-tukey', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))
})

# Multiple random intercepts, covariates, and slopes ----------------------
set.seed(1000)
J.x <- 15
J.y <- 15
J <- J.x * J.y
n.rep <- sample(2:4, J, replace = TRUE)
beta <- c(0, -0.3)
p.abund <- length(beta)
alpha <- c(0.5, -0.2)
p.det <- length(alpha)
mu.RE <- list(levels = c(10, 15),
              sigma.sq.mu = c(0.3, 0.9), 
              beta.indx = c(1, 2))
p.RE <- list(levels = c(25, 15),
             sigma.sq.p = c(0.5, 0.7), 
             alpha.indx = c(1, 2))
phi <- runif(1, 3 / 1, 3 / .1)
sigma.sq <- 0.5
kappa <- 0.5
phi <- 3 / 0.5
nu <- 1
sp <- TRUE
cov.model <- 'matern'
family <- 'NB'
dat <- simNMix(J.x = J.x, J.y = J.y, n.rep = n.rep, beta = beta, alpha = alpha,
	       mu.RE = mu.RE, p.RE = p.RE, sp = TRUE, family = family, kappa = kappa,
               sigma.sq = sigma.sq, phi = phi, nu = nu, cov.model = cov.model)

pred.indx <- sample(1:J, round(J * .25), replace = FALSE)
y <- dat$y[-pred.indx, , drop = FALSE]
y.0 <- dat$y[pred.indx, , drop = FALSE]
# Occupancy covariates
X <- dat$X[-pred.indx, , drop = FALSE]
X.re <- dat$X.re[-pred.indx, , drop = FALSE]
# Prediction covariates
X.0 <- dat$X[pred.indx, , drop = FALSE]
X.re.0 <- dat$X.re[pred.indx, , drop = FALSE]
# Detection covariates
X.p <- dat$X.p[-pred.indx, , , drop = FALSE]
X.p.re <- dat$X.p.re[-pred.indx, , , drop = FALSE]
coords <- as.matrix(dat$coords[-pred.indx, ])
coords.0 <- as.matrix(dat$coords[pred.indx, ])

abund.covs <- cbind(X, X.re)
colnames(abund.covs) <- c('int', 'abund.cov.1', 'abund.factor.1', 'abund.factor.2')
det.covs <- list(det.cov.1 = X.p[, , 2],
                 det.factor.1 = X.p.re[, , 1],
                 det.factor.2 = X.p.re[, , 2])
data.list <- list(y = y,
		  abund.covs = abund.covs,
 		  det.covs = det.covs,
                  coords = coords)
# Priors
prior.list <- list(beta.normal = list(mean = 0, var = 10),
		   alpha.normal = list(mean = 0, var = 2.72),
                   sigma.sq.ig = c(2, 1),
                   phi.unif = c(3 / 1, 3 / .1),
                   nu.unif = c(0.1, 5))

# Starting values
inits.list <- list(alpha = 0,
		   beta = 0,
		   kappa = 1.3,
		   sigma.sq.mu = 0.5,
		   N = apply(y, 1, max, na.rm = TRUE),
                   phi = 3 / .5, sigma.sq = 0.5, nu = 2)
tuning <- list(beta = 0.1, alpha = 0.1, beta.star = 0.1, alpha.star = 0.1,
               kappa = 0.2, phi = 0.5, sigma.sq = 0.5, nu = 0.5, w = 1)
n.batch <- 10
batch.length <- 25
n.burn <- 50
n.thin <- 2
n.chains <- 2
abund.formula <- ~ abund.cov.1 + (1 | abund.factor.1) + (1 | abund.factor.2) + (abund.cov.1 | abund.factor.2)
det.formula <- ~ det.cov.1 + (1 | det.factor.1) + (1 | det.factor.2) + (det.cov.1 | det.factor.2)

out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      inits = inits.list,
	      family = 'NB',
	      tuning = tuning,
	      priors = prior.list,
	      cov.model = 'matern',
	      NNGP = TRUE,
	      n.neighbors = 5,
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 1,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains,
              k.fold = 2,
              k.fold.threads = 1)

# Test to make sure it worked ---------
test_that("out is of class spNMix", {
  expect_s3_class(out, "spNMix")
})

# Check cross-validation --------------
test_that("cross-validation works", {
  expect_equal(length(out$rmspe), 1)
  expect_type(out$rmspe, "double")
  expect_gt(out$rmspe, 0)
})

# Check random effects ----------------
test_that("random effects are empty", {
  expect_equal(out$pRE, TRUE)
  expect_equal(out$muRE, TRUE)
})

test_that("random effect gives error when non-numeric", {
  data.list$det.covs$det.factor.1 <- factor(data.list$det.covs$det.factor.1)
  expect_error(out <- spNMix(abund.formula = abund.formula,
	    det.formula = det.formula,
	    data = data.list,
	    n.batch = n.batch,
	    batch.length = batch.length,
	    inits = inits.list,
	    family = 'NB',
	    tuning = tuning,
	    priors = prior.list,
	    accept.rate = 0.43,
	    n.omp.threads = 1,
	    verbose = FALSE,
	    n.report = 50,
	    n.burn = n.burn,
	    n.thin = n.thin,
	    n.chains = n.chains))
  data.list$det.covs$det.factor.1 <- as.character(factor(data.list$det.covs$det.factor.1))
  expect_error(out <- spNMix(abund.formula = abund.formula,
	    det.formula = det.formula,
	    data = data.list,
	    n.batch = n.batch,
	    batch.length = batch.length,
	    inits = inits.list,
	    family = 'NB',
	    tuning = tuning,
	    priors = prior.list,
	    accept.rate = 0.43,
	    n.omp.threads = 1,
	    verbose = FALSE,
	    n.report = 50,
	    n.burn = n.burn,
	    n.thin = n.thin,
	    n.chains = n.chains))
})


# Check output data output is correct -
test_that("out$y == y", {
  expect_equal(out$y, y)
})

# Check Poisson -----------------------
test_that("Poisson works", {
  out <- spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'spherical',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = FALSE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1)
  expect_s3_class(out, "spNMix")
})

# Check default priors ----------------
test_that("default priors, multiple chains, inits, burn, thin work", {
  out <- spNMix(abund.formula = abund.formula,
	      det.formula = det.formula,
	      data = data.list,
	      n.batch = n.batch,
	      batch.length = batch.length,
	      family = 'NB',
	      NNGP = TRUE,
	      n.neighbors = 5,
	      cov.model = 'exponential',
	      accept.rate = 0.43,
	      n.omp.threads = 1,
	      verbose = FALSE,
	      n.report = 50,
	      n.burn = n.burn,
	      n.thin = n.thin,
	      n.chains = n.chains)
  expect_s3_class(out, 'spNMix')
})

# Check summary -----------------------
test_that("summary works", {
  expect_output(summary(out))
})

# Check verbose -----------------------
test_that("verbose prints to the screen", {
  expect_output(spNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
  	    inits = inits.list,
  	    family = 'Poisson',
            NNGP = TRUE,
            n.neighbors = 5,
            cov.model = 'gaussian',
  	    tuning = tuning,
  	    priors = prior.list,
  	    accept.rate = 0.43,
  	    n.omp.threads = 1,
  	    verbose = TRUE,
  	    n.report = 50,
  	    n.burn = n.burn,
  	    n.thin = n.thin,
  	    n.chains = 1))
})

# Check waicAbund -----------------------
test_that("waicAbund works for spNMix", {
  # as.vector gets rid of names
  waic.out <- as.vector(waicAbund(out, max(out$N.samples) + 10))
  expect_equal(length(waic.out), 3)
  expect_equal(waic.out[3], -2 * (waic.out[1] - waic.out[2]))
})

# Check fitted ------------------------
test_that("fitted works for spNMix", {
  fitted.out <- fitted(out)
  expect_equal(length(fitted.out), 2)
})

# Check predictions -------------------
test_that("predict works for spNMix", {
  X.0 <- cbind(X.0, X.re.0)
  colnames(X.0) <- c('(Intercept)', 'abund.cov.1', 'abund.factor.1', 'abund.factor.2')
  pred.out <- predict(out, X.0, coords.0, verbose = FALSE)
  expect_type(pred.out, "list")
  expect_equal(dim(pred.out$mu.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
  expect_equal(dim(pred.out$N.0.samples), c(out$n.post * out$n.chains, nrow(y.0)))
})
test_that("detection prediction works", {
  X.p.0 <- cbind(dat$X.p[, 1, ], dat$X.p.re[, 1, ])
  colnames(X.p.0) <- c('(Intercept)', 'det.cov.1', 'det.factor.1', 'det.factor.2')
  pred.out <- predict(out, X.p.0, type = 'detection')
  expect_type(pred.out, 'list')
  expect_equal(dim(pred.out$p.0.samples), c(out$n.post * out$n.chains, J))
})

# Check PPCs --------------------------
test_that("posterior predictive checks work for spNMix", {
  n.post.samples <- out$n.post * out$n.chains
  ppc.out <- ppcAbund(out, 'chi-square', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))

  ppc.out <- ppcAbund(out, 'chi-square', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))

  ppc.out <- ppcAbund(out, 'freeman-tukey', 1)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y)))

  ppc.out <- ppcAbund(out, 'freeman-tukey', 2)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, max(n.rep)))

  ppc.out <- ppcAbund(out, 'chi-square', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))

  ppc.out <- ppcAbund(out, 'freeman-tukey', 0)
  expect_type(ppc.out, "list")
  expect_equal(length(ppc.out$fit.y), n.post.samples)
  expect_equal(length(ppc.out$fit.y.rep), n.post.samples)
  expect_equal(dim(ppc.out$fit.y.group.quants), c(5, nrow(y), max(n.rep)))
  expect_equal(dim(ppc.out$fit.y.rep.group.quants), c(5, nrow(y), max(n.rep)))
})


