...
out <- msNMix(abund.formula = abund.formula,
...
test_that("out is of class msNMix", {
  expect_s3_class(out, "msNMix")
})
...
test_that("verbose prints to the screen", {
  expect_output(msNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
...
# Check waicAbund -----------------------
test_that("waicAbund works for msNMix", {
...
# Check fitted ------------------------
test_that("fitted works for msNMix", {
  fitted.out <- fitted(out)
  expect_equal(length(fitted.out), 2)
})

# Check predictions -------------------
test_that("predict works for msNMix", {
  X.0 <- dat$X
...
# Check PPCs --------------------------
test_that("posterior predictive checks work for msNMix", {
...
test_that("random effect gives error when non-numeric", {
  ...
  expect_error(out <- msNMix(abund.formula = abund.formula,
	    det.formula = det.formula,
	    data = data.list,
	    n.batch = n.batch,
	    batch.length = batch.length,
