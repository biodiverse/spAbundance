...
dat <- simMsNMix(J.x = J.x, J.y = J.y, n.rep = n.rep, n.sp = n.sp, beta = beta, alpha = alpha,
	        mu.RE = mu.RE, p.RE = p.RE, sp = TRUE, kappa = kappa, family = family, 
                n.factors = n.factors, factor.model = factor.model, phi = phi,
                nu = nu, cov.model = 'matern')
...
test_that("out is of class sfMsNMix", {
...
test_that("verbose prints to the screen", {
  expect_output(sfMsNMix(abund.formula = abund.formula,
  	    det.formula = det.formula,
  	    data = data.list,
  	    n.batch = n.batch,
  	    batch.length = batch.length,
...
# Check fitted ------------------------
test_that("fitted works for sfMsNMix", {
  fitted.out <- fitted(out)
  expect_equal(length(fitted.out), 2)
})

# Check predictions -------------------
test_that("predict works for sfMsNMix", {
...
test_that("random effect gives error when non-numeric", {
  ...
  expect_error(out <- sfMsNMix(abund.formula = abund.formula,
	    det.formula = det.formula,
	    data = data.list,
	    n.batch = n.batch,
	    batch.length = batch.length,
